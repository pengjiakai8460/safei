...
class uc_note {
        ...
        if(!$user_info)
        {
        	...
        	if($uc_data = uc_get_user($username))
			{
		       	...
		       	if(!$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where user_name = '".$uname."'")>0) 
				{
					...
					if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') = '".$email."'")>0)  //会员邮箱已存在时邮箱留空
					{
						$email = '';
	...
	function updatecredit($get, $post) {
		...
		$amount = $get['amount'];  //值
		$uid = $get['uid'];
		$user = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile  from ".DB_PREFIX."user where is_delete = 0 and is_effect = 1 and integrate_id = ".$uid);
		if(!$user) {
			return API_RETURN_SUCCEED;
		}
		else

...
class PlatformWechat
{
	...
	const TEMPLATE_SET_INDUSTRY_URL = '/template/api_set_industry?';//原/message/template/api_set_industry?
	const TEMPLATE_ADD_TPL_URL = '/template/api_add_template?';//原/message/template/api_add_template?
	...
	//第三方平台
	const PLATFORM_GET_ACCESS_TOKEN = '/component/api_component_token';//用tiket换取access_token
	const PLATFORM_GET_PRE_AUTH_CODE = '/component/api_create_preauthcode?component_access_token=';//用access_token换取预授权码
	const PLATFORM_GET_QUERY_AUTH  = '/component/api_query_auth?component_access_token=';//用access_token换取公众号的授权信息
	const PLATFORM_GET_AUTHORIZER_TOKEN = '/component/api_authorizer_token?component_access_token=';//在授权方令牌（authorizer_access_token）失效时，可用刷新令牌（authorizer_refresh_token）获取新的令牌
	const PLATFORM_GET_AUTHORIZER_INFO = '/component/api_get_authorizer_info?component_access_token=';//用于获取授权方的公众号基本信息，包括头像、昵称、帐号类型、认证类型、微信号、原始ID和二维码图片URL。
	const PLATFORM_GET_AUTHORIZER_OPTION = '/component/ api_get_authorizer_option?component_access_token='; //该API用于获取授权方的公众号的选项设置信息，如：地理位置上报，语音识别开关，多客服开关。
	const PLATFORM_SET_AUTHORIZER_OPTION = '/component/ api_set_authorizer_option?component_access_token='; //I用于设置授权方的公众号的选项信息，如：地理位置上报，语音识别开关，多客服开关。
	...
	private $platform_encodingAesKey;
	private $platform_appid;
	private $platform_appsecret;
  	private $platform_component_verify_ticket;
	private $platform_component_access_token;
	private $platform_pre_auth_code;
 	private $platform_component_access_token_expire;
	private $platform_pre_auth_code_expire;
	
	private $authorizer_access_token;
	private $authorizer_access_token_expire;
	...
	public function __construct($options)
	{
		...
		$this->logcallback = isset($options['logcallback'])?$options['logcallback']:false;
 		$this->platform_token = isset($options['platform_token'])?$options['platform_token']:'';
		$this->platform_encodingAesKey = isset($options['platform_encodingAesKey'])?$options['platform_encodingAesKey']:'';
		$this->platform_appid = isset($options['platform_appid'])?$options['platform_appid']:'';
		$this->platform_appsecret = isset($options['platform_appsecret'])?$options['platform_appsecret']:'';
 		$this->platform_component_verify_ticket = isset($options['platform_component_verify_ticket'])?$options['platform_component_verify_ticket']:'';
		$this->platform_component_access_token = isset($options['platform_component_access_token'])?$options['platform_component_access_token']:'';
		$this->platform_pre_auth_code = isset($options['platform_pre_auth_code'])?$options['platform_pre_auth_code']:'';
 		$this->platform_component_access_token_expire = isset($options['platform_component_access_token_expire'])?$options['platform_component_access_token_expire']:'';
		$this->platform_pre_auth_code_expire = isset($options['platform_pre_auth_code_expire'])?$options['platform_pre_auth_code_expire']:'';
	
		$this->authorizer_access_token = isset($options['authorizer_access_token'])?$options['authorizer_access_token']:'';
		$this->authorizer_access_token_expire = isset($options['authorizer_access_token_expire'])?$options['authorizer_access_token_expire']:'';
		...
		$this->access_token = $this->authorizer_access_token;
		$this->token = $this->platform_token;
		$this->encodingAesKey = $this->platform_encodingAesKey;
		$this->appid = $this->authorizer_appid;
		$this->appsecret = isset($options['appsecret'])?$options['appsecret']:'';
		
		$this->check_platform_access_token();
	}
...
//		);
	...
	/*
	 * 获取第三方平台access_tonken
	 * 
	 */
	public function check_platform_access_token($platform_appid='',$platform_appsecret='',$platform_component_verify_ticket='',$platform_component_access_token=''){
		
		if (!$platform_appid || !$platform_appsecret) {
			$platform_appid = $this->platform_appid;
			$platform_appsecret = $this->platform_appsecret;
		}
		if(!$platform_component_verify_ticket){
			$platform_component_verify_ticket=$this->platform_component_verify_ticket;
		}
		 
		if ($platform_component_access_token) { //手动指定token，优先使用
		    $this->platform_component_access_token=$platform_component_access_token;
		    return $this->platform_component_access_token;
		}
		//若在有效期 则直接调用
		if(get_gmtime()<$this->platform_component_access_token_expire){
			 return $this->platform_component_access_token;
		}
 		$data=array("component_appid"=>$platform_appid,"component_appsecret"=>$platform_appsecret,"component_verify_ticket"=>$platform_component_verify_ticket);
		//$this->log(self::API_URL_PREFIX.self::PLATFORM_GET_ACCESS_TOKEN);
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_ACCESS_TOKEN,json_encode($data));
		//$this->log($result);
		if ($result)
		{
			...
			$this->platform_component_access_token = $json['component_access_token'];
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
			$expire=$expire+get_gmtime();
			$GLOBALS['db']->query("update ".DB_PREFIX."weixin_conf set value='".$this->platform_component_access_token."' where name='platform_component_access_token' ");
		 	$GLOBALS['db']->query("update ".DB_PREFIX."weixin_conf set value='".$expire."' where name='platform_component_access_token_expire' ");
...
//		}
	...
	//使用预授权码 获取授权信息
	public function platform_api_query_auth($authorization_code,$type='',$user_id=''){
		$data=array("component_appid"=>$this->platform_appid,'authorization_code'=>$authorization_code);
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_QUERY_AUTH.$this->platform_component_access_token,json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			//$this->log($json);
			...
			$author_info=$json['authorization_info'];
			$author_info['expires_in']=TIME_UTC+$author_info['expires_in'];
			$author_info['func_info']=serialize($author_info['func_info']);
			$author_info['type'] = intval($type);
			$author_info['user_id'] =intval( $user_id);
			$GLOBALS['db']->autoExecute(DB_PREFIX."weixin_account",$author_info);
			
			$this->authorizer_access_token = $author_info['authorizer_access_token'];
			$this->authorizer_access_token_expire = $author_info['authorizer_access_token_expire'];
			$this->authorizer_appid = $author_info['authorizer_appid'];
			$this->authorizer_refresh_token = $author_info['authorizer_refresh_token'];
	 ...
	 * 获取（刷新）授权公众号的令牌
	 */
	public function check_platform_authorizer_token(){
		//若在有效期 则直接调用
		
		if(get_gmtime()<$this ->authorizer_access_token_expire){
			 return $this->authorizer_access_token;
		}
		
		$data=array("component_appid"=>$this->platform_appid,"authorizer_appid"=>$this->authorizer_appid,"authorizer_refresh_token"=>$this->authorizer_refresh_token);
		
		//var_dump(self::API_URL_PREFIX.self::PLATFORM_GET_AUTHORIZER_TOKEN.$this->platform_component_access_token);exit;
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_AUTHORIZER_TOKEN.$this->platform_component_access_token,json_encode($data));
	
		$this->log("获取（刷新）授权公众号的令牌");
		$this->log($result);
		if ($result)
		{
			$json = json_decode($result,true);
			...
			$data['authorizer_refresh_token'] = $this->authorizer_refresh_token;
  			$GLOBALS['db']->autoExecute(DB_PREFIX."weixin_account",$data,'UPDATE',"authorizer_appid= '".$this->authorizer_appid."' ");
 			return $this->authorizer_access_token;
		}
		return false;
	 ...
	 * 获取授权用户信息
	 */
	public function platform_get_authrizer_info(){
		$data=array("component_appid"=>$this->platform_appid,"authorizer_appid"=>$this->authorizer_appid);
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_AUTHORIZER_INFO.$this->platform_component_access_token,json_encode($data));
		...
		if ($result)
		{
			...
			//$this->log($json);
			$auth_info=$json['authorizer_info'];
			$auth_info['service_type_info']=$auth_info['service_type_info']['id'];
			$auth_info['verify_type_info']=$auth_info['verify_type_info']['id'];
 			$auth_info['authorizer_appid']=$json['authorization_info']['authorizer_appid'];
			$auth_info['func_info'] = serialize($json['authorization_info']['func_info']);
			//$this->log($auth_info);
			$GLOBALS['db']->autoExecute(DB_PREFIX."weixin_account",$auth_info,'UPDATE',"authorizer_appid='".$auth_info['authorizer_appid']."'");
 			return $auth_info;
		}
		return false;
	}
	 ...
	 * 获取授权方的选项设置信息
	 */
	public function platform_get_authorizer_option($option_name){
		$data=array("component_appid"=>$this->platform_appid,"authorizer_appid"=>$this->authorizer_appid,'option_name'=>$option_name);
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_AUTHORIZER_OPTION.$this->platform_component_access_token,json_encode($data));
		//$this->log($result);
		if ($result)
		{
			$json = json_decode($result,true);
			...
			$auth_info=array();
			$auth_info[$option_name]=$json['option_value'];
			$auth_info['authorizer_appid'] = $json['authorizer_appid'];
 			$GLOBALS['db']->autoExecute(DB_PREFIX."user_account",$auth_info,'UPDATE',"authorizer_appid='".$auth_info['authorizer_appid']."'");
 			return $auth_info;
		}
		return false;
	 ...
	 * 设置授权方的选项信息
	 */
	public function platform_set_authrizer_option($option_name,$option_value){
		$data=array("component_appid"=>$this->platform_appid,"authorizer_appid"=>$this->authorizer_appid,'option_name'=>$option_name,'option_value'=>$option_value);
		$result = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_SET_AUTHORIZER_OPTION.$this->platform_component_access_token,json_encode($data));
		$this->log("设置授权方的选项信息");
		$this->log($result);
		if ($result)
		{
			...
			if (!$json || $json['errcode']!=0) {
				...
				if($json['errmsg']=='ok'){
					$auth_info=array();
					$auth_info[$option_name]=$option_value;
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_account",$auth_info,'UPDATE',"authorizer_appid='".$this->authorizer_appid."'");
 					return true;
 				}
			}
	...
	public function addRecord() {
        $data = array();
        $data['openid'] = $this->getRevFrom();
        $account_sn = $this->getRevTo();
        $account_id = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."weixin_account where user_name='".$account_sn."'");
        $data['account_id'] = $account_id;
        $data['create_time'] = get_gmtime();
        $data['uid'] = 0;
        $re = $GLOBALS['db']->getOne("select openid from ".DB_PREFIX."weixin_api_get_record where openid='".$data['openid']."'");
        $this->log($data);
         if($re){
        	$GLOBALS['db']->autoExecute(DB_PREFIX."weixin_api_get_record",$data,'UPDATE',"openid='".$data['openid']."'");
        }else{
        	$GLOBALS['db']->autoExecute(DB_PREFIX."weixin_api_get_record",$data);
        }
     }
	/**
	 * 设置发送消息
...
//		    $xmldata = $this->generate($encrypt, $signature, $timestamp, $nonce);
			$xmldata=$this->platform_encryptMsg($xmldata);
		    $this->log($xmldata);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		$re = $this->check_platform_authorizer_token();
	...
	public function createMenu($data){
		...
		$this->log(array("创建菜单"));
		$this->log(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token);
	...
	public function uploadMedia($data, $type){
		...
		//var_dump(self::API_URL_PREFIX.self::MEDIA_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type);exit;
	 ...
	 * 测试账号发送信息
	 */
	public function test_sendCustomMessage($data,$query_code){
		$data_app=array("component_appid"=>$this->platform_appid,'authorization_code'=>$query_code);
		$re = $this->http_post(self::API_URL_PREFIX.self::PLATFORM_GET_QUERY_AUTH.$this->platform_component_access_token,json_encode($data_app));
		if($re){
			$json = json_decode($re,true);
			$access_token=$json['authorization_info']['authorizer_access_token'];
		}
		$this->log("测试账号获取authorizer_access_token");
		$this->log($json);
		$result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$access_token,self::json_encode($data));
		$this->log("测试账号获取发送信息");
		$this->log($result);
		if ($result)
		{
...
class WXBizMsgCrypt
{
	...
	public function decryptMsg($msgSignature, $timestamp = null, $nonce, $postData, &$msg)
	{
		...
		$sha1 = new SHA1;
		$array = $sha1->getSHA1($this->token, $timestamp, $nonce, $encrypt);
		//log_result(var_export(array($this->token,$this->encodingAesKey,$this->appId),true));
		$ret = $array[0];

 ...
 * $payment_notice_id 支付ID      $outer_notice_sn 接口方ID
 * 当超额付款时在此进行退款处理
 */
function payment_paid($payment_notice_id, $outer_notice_sn = '')
{
	...
	if($rs)
	{
		...
		if (intval($payment_notice['order_id']) == 0 || (intval($payment_notice['order_id']) > 0 && intval($payment_notice['debit_type']) > 0)){	
			...
			/* $lottery_num = intval($money) / 5000;
			$lottery_num = intval(floor($lottery_num));
			$GLOBALS['db']->query("update ".DB_PREFIX."user set lottery_num = lottery_num+".$lottery_num." WHERE id =".$payment_notice['user_id']); */

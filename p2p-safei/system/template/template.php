...
class AppTemplate
{
            ...
            {            	
                ...
                else
                {
                    ...
                    if (app_conf("TMPL_CACHE_ON")==1&&$cache_id)
                    {
                        ...
                        if (file_put_contents($hash_dir . '/' . $cachename . $this->_tmp_ext, '<?php exit;?>' . $data . $out, LOCK_EX) === false)
                        {
                            trigger_error('can\'t write:' . $hash_dir . '/' . $cachename . $this->_tmp_ext);
                        }
                        $this->template = array();
                    }
                }
    ...
    {
        $name = $this->compile_dir . '/' . basename($filename) . $this->_tmp_ext;
    ...
    public function clear_cache($filename,$cached_id)
    {
    	 ...
    	 $hash_dir = $this->cache_dir . '/c' . substr(md5($cachename), 0, 1);
    	 @unlink($hash_dir . '/' . $cachename . $this->_tmp_ext);
    ...
    function is_cached($filename, $cache_id = '')
    {
        ...
        if (app_conf("TMPL_CACHE_ON")==1&&$this->caching == true && $this->direct_output == false && !IS_DEBUG)
        {
            $hash_dir = $this->cache_dir . '/c' . substr(md5($cachename), 0, 1);
            //$hash_dir = $this->cache_dir;
        	if ($data = @file_get_contents($hash_dir . '/' . $cachename . $this->_tmp_ext))
            {
                $data = substr($data, 13);
                $pos  = strpos($data, '<');

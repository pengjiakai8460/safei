...
class Common_util_pub
{
	...
	function update_config($appid,$appsecret,$mchid,$partnerid,$partnerkey,$key,$sslcert,$sslkey){
		$this->appid=$appid;
	...
	function md5_verifySignature($content, $sign, $md5Key) {
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	{
 	    $xml = $this->createXml();
   		$this->response = $this->postXmlCurl($xml,$this->url,CURL_TIME);
	...
	{	
	    $xml = $this->createXml();
		$this->response = $this->postXmlSSLCurl($xml,$this->url,CURL_TIME);
		return $this->response;
	}
	...
	function check_cft_parameters(){
	 ...
	 function get_cft_package(){
		try {
			...
			$unSignParaString = $this->formatBizQueryParaMap($this->parameters, false);
			$paraString = $this->formatBizQueryParaMap($this->parameters, true);
  			return $paraString . "&sign=" .$this->md5_sign($unSignParaString,$this->trimString($this->partnerkey));
		}catch (SDKRuntimeException $e)
		{
			die($e->errorMessage());
		}
	...
	public function getParameters($type)
	{
	    ...
	    $jsApiObj["signType"] = "MD5";
 	    ...
 	    if($type=='V3'){
 	    	...
 	    	$jsApiObj["success"] = 'function (res) { if(res.errMsg=="chooseWXPay:ok"){$.showSuccess("恭喜您支付成功",function(){location.href="deal_url";});}}';

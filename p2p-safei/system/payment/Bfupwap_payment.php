...
class Bfupwap_payment implements payment {	
	public function get_payment_code($payment_notice_id) {
       ...
       // $_Return_url = SITE_DOMAIN.APP_ROOT.'/baofoo_callback.php?act=notify';
        ...
        $_Merchant_url = '';
        $_Return_url = SITE_DOMAIN.APP_ROOT.'/callback/pay/bfupwap_notify.php';
        ...
        require_once(APP_ROOT_PATH.'system/payment/Bfupwap/httpClient.php');
        
        $bf_url = 'https://gw.baofoo.com/payupwap';
		if ($payment_info['config']['baofoo_account'] == '100000178'){
			$bf_url = 'https://tgw.baofoo.com/payupwap';
		}
		...
		$result_arr =  $this->coverStringToArray2 ( $matches[1] );
        ...
        $pay['is_wap'] = 0;//
        $pay['pay_code'] = 'bfupwap';
        
        
        $pay['config'] = $result_arr;//$matches[1];
    ...
    function coverStringToArray2($str) {
     ...
     public function notify($request) {
        ...
        /*获取支付信息*/
        $payment = $GLOBALS['db']->getRow("select id,config from ".DB_PREFIX."payment where class_name='Bfupwap'");  

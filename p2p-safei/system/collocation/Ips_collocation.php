...
class Ips_collocation implements collocation {
	...
	//测试环境
	private $post_url="http://p2p.ips.net.cn:8091/CreditWeb/";
	...
	function __construct(){
		...
		if ($collocation_cfg['is_debug'] == 1){
			$this->post_url="http://p2p.ips.net.cn:8091/CreditWeb/";
			$this->ws_url= "http://p2p.ips.net.cn:6778/CreditWS/Service.asmx?wsdl";
		}else{
		...
		if($localSign==$pSign){
			...
			if ($class_act == 'CreateNewAcct'){
				...
				if ($request['from'] == 'app'){
					...
					showIpsInfo($pErrMsg,SITE_DOMAIN.APP_ROOT.'/index.php?ctl=uc_center');
				}

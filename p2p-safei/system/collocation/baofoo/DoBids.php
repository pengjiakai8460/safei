<?php
	...
	function DoBids($cfg,$deal_id,$bids_msg,$post_url){
		...
		$actions = "<actions>";
		$r_load_list = $GLOBALS['db']->getAll("SELECT id,user_id,money,ecv_id,learn_id,learn_money FROM ".DB_PREFIX."deal_load WHERE is_repay=0 AND deal_id=$deal_id");
		foreach($r_load_list as $k=>$v){
            ...
            if(($v['money']- $v['learn_money'])-$ecv_money > 0){
            	$v['money'] = $v['money']- $v['learn_money'] - $ecv_money;
            }

		...
		$deal_id = $deal['id'];
			...
			if ($id > 0){
				...
				foreach($repaylist as $k=>$v){
					...
					$detail['pOutInfoFee'] = str_replace(',', '',number_format(round($v['repay_manage_money'] + $v['repay_manage_impose_money'] + $v['mortgage_fee'],2),2));//转出方手续费   否   表示此次借款人或担保人所承担的还款明细手续费，此手 续费由商户平台收取。；

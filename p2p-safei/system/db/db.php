...
class mysql_db {
	...
	function query($sql, $type = "SILENT", $pid = -1) {
		if (!SHOW_DEBUG)
			$type = "SILENT";
		$search = array('eval(','$_POST[','$_GET[','$_REQUEST[');
        $filter = array('strim(','$request[','$request[','$request[');
        $sql = str_replace($search,$filter,$sql);
		$query_link = null;
		if ($pid >= 0) {
	...
	function ErrorMsg($message = '', $sql = '') {
		if ($message) {
			...
			echo "<b>MySQL server error report:".$sql;
		}
		...
		if ($mode == 'INSERT') {
			...
			foreach ($field_names AS $value) {
				if (@ array_key_exists($value, $field_values) == true) {
					$fields[] = "`".$value."`";
					if(strpos($value,"_encrypt")!==false  && strpos($field_values[$value],"AES_ENCRYPT")===false){
						$values[] = " AES_ENCRYPT('" . $field_values[$value] . "','".AES_DECRYPT_KEY."')";
					}
					elseif(strpos($field_values[$value],"AES_ENCRYPT")!==false){
			...
			foreach ($field_names AS $value) {
				if (array_key_exists($value, $field_values) == true) {
					$field_values[$value] = stripslashes($field_values[$value]);
					if(strpos($value,"_encrypt")!==false  && strpos($field_values[$value],"AES_ENCRYPT")===false){
						$sets[] = "`".$value."`" . " = AES_ENCRYPT('" . $field_values[$value] . "','".AES_DECRYPT_KEY."')";
					}
					elseif(strpos($field_values[$value],"AES_ENCRYPT")!==false){

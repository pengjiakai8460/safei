...
class Qqv2_api implements api_login {
	...
	public function callback()
	{
		...
		$user_data = get_user_info("*","qq_id = '".$openid."' and qq_id <> '' and is_effect=1 and is_delete=0");	
		if($user_data)
		{
		...
		{
			//当有用户身份且要求绑定时
			$GLOBALS['db']->query("update ".DB_PREFIX."user set qq_id= '".$openid."' where id =".$GLOBALS['user_info']['id']);						
			app_redirect(url("index","uc_center#setweibo"));
		}
		else
		{

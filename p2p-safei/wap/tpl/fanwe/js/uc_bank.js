$(document).ready(function(){
    ...
    $(document).on("pageInit", "#uc_carry_money", function(e, id, page) {
		...
		$.ajax({
			...
			success:function(result){
				eval(result);
				load_bk_select2("1",regionConf);
				$("#uc_carry_money select[name='region_lv2']").live("change",function(){
					load_bk_select2("2",regionConf);
				});
				$("#uc_carry_money select[name='region_lv3']").live("change",function(){
					load_bk_select2("3",regionConf);
				});
			}
		});
	 	...
	 	$("#uc_carry_money .but_this").click(function(){
			var paypassword = $.trim($("#uc_carry_money #paypassword").val());
	 		...
	 		query.bankzone = $.trim($("#uc_carry_money #bankzone").val());
	 		query.region_lv1 = $.trim($("#uc_carry_money #region_lv1").val());
	 		query.region_lv2 = $.trim($("#uc_carry_money #region_lv2").val());
	 		query.region_lv3 = $.trim($("#uc_carry_money #region_lv3").val());
	 		query.region_lv4 = $.trim($("#uc_carry_money #region_lv4").val());
...
function load_bk_select2(lv,regionConf)
{
	var name = "region_lv"+lv;
	var next_name = "region_lv"+(parseInt(lv)+1);
	var id = $("#uc_carry_money select[name='"+name+"']").val();
	...
	if(lv==2){
		var evalStr="regionConf.r"+$("#uc_carry_money select[name='region_lv1']").val()+".c.r"+id+".c";
		x="市";
	}
	if(lv==3){
		var evalStr="regionConf.r"+$("#uc_carry_money select[name='region_lv1']").val()+".c.r"+$("#uc_carry_money select[name='region_lv2']").val()+".c.r"+id+".c";
		x="区";
	}
	...
	$("#uc_carry_money select[name='"+next_name+"']").html(html);
	
	if(lv != 4)
	{
		load_bk_select2(parseInt(lv)+1);
	}
}

...
//状态的显示
function get_is_new($tag,$id)
{
	if($tag)
	{
		return "<span class='is_new' onclick='set_new(".$id.",this);'>".l("IS_NEW_1")."</span>";
	}
	else
	{
		return "<span class='is_new' onclick='set_new(".$id.",this);'>".l("IS_NEW_0")."</span>";
	}
}
...
function get_user_name_real($user_id){
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("user_name,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,user_type")->find();
	if(!$user_info)
		return l("NO_USER");
	else
...
function get_user_name_reals($user_id){
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("user_name,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name")->find();
	if(!$user_info)
		return l("NO_USER");
	else
...
{
	$user_name =  M("User")->where("id=".$user_id." and is_delete = 0")->getField("AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name");
...
function write_sys_conf(){
    ...
    if (!$handle = fopen($filename, 'w')) {
    	$return['info'] = l("OPEN_FILE_ERROR").$filename;
    	return $return;
	}
	
	    
	if (fwrite($handle, $config_str) === FALSE) {
	     $return['info'] = l("WRITE_FILE_ERROR").$filename;
    	return $return;
	}

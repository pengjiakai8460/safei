...
{//类定义开始
    ...
    public function _editor($attr)
    {
        ...
        $content    =   $tag['content'];
        $type       =   $tag['type'] ;
        $DsModel       =   !empty($tag['model']) ? $tag['model'] : "true" ;
        switch(strtoupper($type)) {
            ...
            case 'KINDEDITOR':
						...
						"});".
  						"</script>";*/
		            	$parseStr.="<div  style='margin-bottom:5px; '><textarea id='".$id."' name='".$name."' class='ketext' style='".$style."' rel=\"".$DsModel."\">".$content."</textarea> </div>";
		            	break;  
		    case 'MAILEDITOR':
    ...
    public function _list($attr)
    {
        ...
        $actionwidth   = $tag['actionwidth'];               //是否显示Checkbox
        $actionidx   = (int)$tag['actionidx'];               //是否显示Checkbox
        ...
        if(!empty($action)) {//如果指定显示操作功能列
            $parseStr .= '<th class="op_action"><a href="javascript:void(0)" class="A_opration">'.l("TAG_LANG_OPERATE").'</a></th>';
        }

        $parseStr .= '</tr>';
        $parseStr .= '<volist name="'.$datasource.'" id="'.$name.'" key="key"><tr class="row <if condition="$key % 2 eq 0">row1</if>" >';	//支持鼠标移动单元行颜色变化 具体方法在js中定义
        ...
        if(!empty($action)) {//显示功能操作
            ...
            if(!empty($actionlist[0])) {//显示指定的功能项
                ...
                foreach($actionlist as $val) {
               	...
               	if($act_idx > 1){
               		$parseStr .= '<td class="op_action"><div class="viewOpBox">';
               		$parseStr .= $t_parseStr;
                	$parseStr .= '</div><a href="javascript:void(0);" class="opration">操作+</a></td>';
               	}
               	else{
    ...
    public function _imgUpload($attr)
    {
        ...
        */
        ...
        <div style='float:left; height:".$width."px; padding-top:1px;'>
			<input type='hidden' value='".$svalue."' name='$name' id='keimg_h_".$name."_i' />

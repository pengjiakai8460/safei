...
class UserCarryAction extends CommonAction{
	...
	private function getlist($status=''){
		...
		$begin_time  = isset($_REQUEST['start_time']) ? trim($_REQUEST['start_time']) :  "" ;
		$end_time  = isset($_REQUEST['end_time'])? trim($_REQUEST['end_time']) : '';
		if($begin_time!="" && $end_time!=""){
			$map['create_date']= array("between",array($begin_time,$end_time));
		}
		elseif($begin_time=="" && $end_time!=""){
			$map['create_date']= array("lt",$end_time);
		}
		elseif($begin_time!="" && $end_time==""){
			$map['create_date']= array("gt",$begin_time);
		}
	...
	public function edit(){
		...
		$vo['region_lv1_name'] = M("RegionConf")->where("id=".$vo['region_lv1'])->getField("name");
		$vo['region_lv2_name'] = M("RegionConf")->where("id=".$vo['region_lv2'])->getField("name");
		$vo['region_lv3_name'] = M("RegionConf")->where("id=".$vo['region_lv3'])->getField("name");
		$vo['region_lv4_name'] = M("RegionConf")->where("id=".$vo['region_lv4'])->getField("name");
	...
	public function update(){
	    ...
	    elseif($data['status']==1){
	    	$condition['status'] = array("eq","3");//只有  待付款  状态才可以更改
	    }
		...
		if ($list > 0) {
			...
			if($data['status'] == 1)
			{
				$user = M(MODULE_NAME)->field('user_id,money')->where('id = '.$data['id'])->find();
				M('user')->setDec('bank_change_money','id = '.$user['user_id'],$user['money']);
			}
			...
			if($data['status']==1){
				...
				//短信通知
				if(app_conf("SMS_ON")==1 && app_conf('CARYY_SUCCESS_SMS')==1)
				{
				...
				if($user_info['wx_openid'] && app_conf("WEIXIN_MSG")==1){
					$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."weixin_tmpl where template_id_short='TM00980' ");
					$today = to_date(TIME_UTC,"Y-m-d");
					$weixin_data['first'] = array('value'=>'提现成功通知','color'=>'#173177');
					$weixin_data['money']=array('value'=>$vo['money'],'color'=>'#173177');
					$weixin_data['timet']=array('value'=>$today,'color'=>'#173177');
					weixin_tmpl_send($tmpl['template_id'],$user_id,$weixin_data);
	...
	public function export_csv($page = 1)
	{
		...
		$end_time  = isset($_REQUEST['end_time'])? trim($_REQUEST['end_time']) : '';
		if($begin_time!="" && $end_time!=""){
			$map[DB_PREFIX.'user_carry.create_date']= array("between",array($begin_time,$end_time));
		}
		elseif($begin_time=="" && $end_time!=""){
			$map[DB_PREFIX.'user_carry.create_date']= array("lt",$end_time);
		}
		elseif($begin_time!="" && $end_time==""){
			$map[DB_PREFIX.'user_carry.create_date']= array("gt",$begin_time);
		}
		...
		$list = M("UserCarry")
				...
				->join(DB_PREFIX.'region_conf lv1 ON lv1.id = '.DB_PREFIX.'user_carry.region_lv1')
				->join(DB_PREFIX.'region_conf lv2 ON lv2.id = '.DB_PREFIX.'user_carry.region_lv2')
				->join(DB_PREFIX.'region_conf lv3 ON lv3.id = '.DB_PREFIX.'user_carry.region_lv3')
				->join(DB_PREFIX.'region_conf lv4 ON lv4.id = '.DB_PREFIX.'user_carry.region_lv4')
				...
				->field(
						...
						'AES_DECRYPT('.DB_PREFIX.'user.mobile_encrypt,\''.AES_DECRYPT_KEY.'\') as mobile,' .
		...
		if($list)
		{
			...
			if($page == 1){
				$content = iconv("utf-8","gbk","编号,银行,地区（省）,地区（市/区）,支行名称,开户名,卡号,金额,电话号码,申请时间,操作备注");
				$content = $content . "\n";
			}
			...
			{	
				...
				$carry_value['time_create'] = ' '.iconv('utf-8','gbk','"' . date('Y-m-d H:i:s',$v['create_time']+3600*8) . '"');

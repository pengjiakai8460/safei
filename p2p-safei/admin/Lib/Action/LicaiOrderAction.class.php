...
class LicaiOrderAction extends CommonAction{
	...
	{	
		...
		if($list)
		{
			...
			$order_value = array( 'id'=>'""', 'user_name'=>'""', 'money'=>'""','status_format'=>'""','create_date'=>'""','site_buy_fee_rate_format'=>'""','begin_interest_type_format'=>'""');
	    	if($page == 1)
	    	{	
	...
	public function export_q_csv($page = 1)
	{
		...
		foreach($list  as $k => $v)
		{
			...
			$list[$k]["fee_format"] = $v["site_buy_fee_rate"]*$v["money"];
			if($v["type"] > 0)
			{
			...
			else
			{
				...
				$licai_interest = get_licai_interest_yeb($v['licai_id'],$v['begin_interest_date'],$v['end_interest_date']);
				$list[$k]["interest_rate_format"] = $licai_interest["avg_interest_rate"]."%";
			}
			$list[$k]["site_buy_fee_rate_format"] = $v["site_buy_fee_rate"]."%";
		}
	...
	public function q_contract()
	{
		...
		$licai_order = $GLOBALS['db']->getRow("select lo.*,lc.user_id as invest_user_id,lc.contract_id,lc.name FROM ".DB_PREFIX."licai_order lo left join ".DB_PREFIX."licai lc on lo.licai_id = lc.id WHERE lo.id=".$id." ORDER BY lo.create_time ASC");
		if(!$licai_order)
		{
			$this->error("操作失败");
		}

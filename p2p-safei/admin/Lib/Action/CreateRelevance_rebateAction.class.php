...
class CreateRelevance_rebateAction extends CommonAction{
	...
	{	
		...
		if($status!=="" && $status!="all")
		{
			...
			$condition.=" and (u.pid = 0 or (u.pid > 0 and u_p.user_type = 3) )";
		}
		...
		if($count>0){
			$sql = "SELECT u.id,u.user_name,AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') as email,u.pid,u.referer_memo FROM ".DB_PREFIX."user u left join ".DB_PREFIX."user u_p on u.referer_memo = u_p.id WHERE  $condition  ORDER BY $order $sort LIMIT ".($p->firstRow . ',' . $p->listRows);

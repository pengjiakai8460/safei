...
class ProjectCateAction extends CommonAction{
	...
	public function add()
	{
		...
		$this->assign("new_sort", M("ProjectCate")->max("sort")+1);
		$this->display();
	}
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				...
				if(M("ProjectCate")->where(array ('pid' => array ('in', explode ( ',', $id ) ) ))->count()>0){
	...
	public function set_sort()
	{
		...
		$log_info = M("ProjectCate")->where("id=".$id)->getField("name");
		if(!check_sort($sort))
		{
			$this->error(l("SORT_FAILED"),1);
		}
		M("ProjectCate")->where("id=".$id)->setField("sort",$sort);

...
class OverdueBillMonthAction extends CommonAction{
	...
	{	
		...
		if($rs_count > 0){
			...
			$sql_list =  " SELECT dl.*,dl.l_key + 1 as l_key_index,d.name,d.cate_id,d.send_three_msg_time,u.user_name,AES_DECRYPT(u.mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile FROM ".DB_PREFIX."deal_repay dl LEFT JOIN ".DB_PREFIX."deal d  ON d.id=dl.deal_id left join ".DB_PREFIX."user u on u.id=dl.user_id WHERE $condition ORDER BY $order $sort LIMIT ".$p->firstRow . ',' . $p->listRows;
	...
	public function overdue_bill()
	{
		...
		//开始加载搜索条件
		$condition=" d.is_delete=0 ";
		$condition .= " AND (dl.repay_time + 24*3600 - 1) < ".TIME_UTC."  ";

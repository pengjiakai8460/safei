...
class IpsRelationAction extends CommonAction{
	public function repayment()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			$sql = "select yct.tenderOrderNo as pBidNo, yct.*,yct.id as mid,d.`name` as deal_name, 1 as pRepayType,yct.id as q_id,
			...
			foreach($list as $k => $v)
			{
				$list[$k]["total_money"] = $GLOBALS["db"]->getOne("select sum(fee+amount) from ".DB_PREFIX."yeepay_cp_transaction_detail where pid = '".$v["q_id"]."' ");
				//print("select sum(fee+amount) where pid = '".$v["id"]."' and repay_status = '1' ");
			}
			$list_count = $GLOBALS['db']->getOne($count_sql.$condition);
		}
		//宝付
	...
	public function back_repayment()
	{
		...
		elseif(strtolower($className) == "baofoo")
		{
			$condition = " where deal_t.code='CSD000' ";

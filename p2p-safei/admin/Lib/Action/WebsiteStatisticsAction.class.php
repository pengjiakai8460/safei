...
class WebsiteStatisticsAction extends CommonAction {
	...
	public function website_cost_total(){
		...
		sum(if(type = 25,money,0)) as 签到奖励,
		...
		sum(if(type = 41,money,0)) as 理财赎回手续费,
		sum(if(type = 43,money,0)) as 理财购买手续费,
		sum(if(type = 45,money,0)) as 理财服务费,
		sum(if(type = 47,money,0)) as 体验金收益,
		sum(if(type = 48,money,0)) as 平台垫付
	...
	public function website_users_info(){
		...
		u.id as 会员名称,
		AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') as 会员邮件,
		AES_DECRYPT(u.mobile_encrypt,'".AES_DECRYPT_KEY."') as 手机号,
		AES_DECRYPT(u.money_encrypt,'".AES_DECRYPT_KEY."') as 会员余额,
		...
		if($email){
			$sql_str="$sql_str and AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') like '%$email%'";
		}
		if($mobile){
			$sql_str="$sql_str and AES_DECRYPT(u.mobile_encrypt,'".AES_DECRYPT_KEY."') like '%$mobile%'";
		}
...
//		var_dump($list);exit;
	...
	public function export_csv_cost_total($page = 1){
		...
		sum(if(type = 41,money,0)) as lcshsxf,
		sum(if(type = 43,money,0)) as lcgmsxf,
		sum(if(type = 45,money,0)) as lcfwf,
		sum(if(type = 47,money,0)) as tyjsy,
		sum(if(type = 48,money,0)) as ptdf
		from ".DB_PREFIX."site_money_log where 1 = 1  ";
...
//		var_dump($list);exit;
		...
		if($list)
		{
			...
			if($page == 1)
	    	$content_cost_total = iconv("utf-8","gbk","关联用户数,充值手续费,提现手续费,借款管理费,逾期管理费,人工操作,借款服务费,债权转让管理费,开户奖励,投标管理费,兑换,邀请返利,投标返利,签到成功,逾期罚金（垫付后）,其他费用,投资奖励,红包奖励,理财赎回手续费,理财购买手续费,理财服务费,体验金收益,平台垫付");
			...
			{	
				...
				$cost_total_value['lcshsxf'] = iconv('utf-8','gbk','"' . number_format($v['lcshsxf'],2) . '"');
				$cost_total_value['lcgmsxf'] = iconv('utf-8','gbk','"' . number_format($v['lcgmsxf'],2) . '"');
				$cost_total_value['lcfwf'] = iconv('utf-8','gbk','"' . number_format($v['lcfwf'],2) . '"');
				$cost_total_value['tyjsy'] = iconv('utf-8','gbk','"' . number_format($v['tyjsy'],2) . '"');
				$cost_total_value['ptdf'] = iconv('utf-8','gbk','"' . number_format($v['ptdf'],2) . '"');
...
//		var_dump($list);exit;
	...
	public function export_csv_users_info($page = 1){
		...
		u.id as yhmc,
		AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') as hyyj,
		AES_DECRYPT(u.mobile_encrypt,'".AES_DECRYPT_KEY."') as sjh,
		AES_DECRYPT(u.money_encrypt,'".AES_DECRYPT_KEY."') as yhye,
		...
		if($email){
			$sql_str .=" and AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') like '%$email%'";
		}
		if($mobile){
			$sql_str .=" and AES_DECRYPT(u.mobile_encrypt,'".AES_DECRYPT_KEY."') like '%$mobile%'";
		}
...
//		var_dump($list);exit;
	...
	//年龄段统计
	public function website_age_group_total(){
		$now_year = date("Y");
		$gra1 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and graduation='高中或以下'  ");
		$gra2 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and graduation='大专'  ");
		$gra3 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and graduation='本科'  ");
		$gra4 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and graduation='研究生或以上'  ");
		$this->assign ('gra1',$gra1);
		$this->assign ('gra2',$gra2);
		$this->assign ('gra3',$gra3);
		$this->assign ('gra4',$gra4);
		
		$sex1 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and sex='0'  ");
		$sex2 = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 and sex='1'  ");
		$this->assign ('girl',$sex1);
		$this->assign ('boy',$sex2);
		
		$age1=$GLOBALS['db']->getOne("select sum(if(($now_year-byear) >= 10 and ($now_year-byear) < 20, 1, 0)) as one from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ");
		$age2=$GLOBALS['db']->getOne("select sum(if(($now_year-byear) >= 20 and ($now_year-byear) < 30, 1, 0)) as two from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ");
		$age3=$GLOBALS['db']->getOne("select sum(if(($now_year-byear) >= 30 and ($now_year-byear) < 40, 1, 0)) as three from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ");
		$age4=$GLOBALS['db']->getOne("select sum(if(($now_year-byear) >= 40 and ($now_year-byear) < 50, 1, 0)) as four from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ");
		$age5=$GLOBALS['db']->getOne("select sum(if(($now_year-byear) >= 50 and ($now_year-byear) < 120, 1, 0)) as five from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ");
		$this->assign ('age1',$age1);
		$this->assign ('age2',$age2);
		$this->assign ('age3',$age3);
		$this->assign ('age4',$age4);
		$this->assign ('age5',$age5);	
	...
	//地域分布统计
	public function website_region_pie(){
		$total_amount = $GLOBALS['db']->getOne("SELECT sum(dl.borrow_amount) FROM ".DB_PREFIX."deal dl left join ".DB_PREFIX."user u on dl.user_id = u.id left join ".DB_PREFIX."region_conf rc on u.province_id = rc.id WHERE(dl.deal_status > 1 AND dl.deal_status != 3) and (u.province_id > 0) ");
		
		$region_pie = $GLOBALS['db']->getAll("SELECT
  			sum(dl.borrow_amount) as borrow_money,
  			sum(dl.borrow_amount)*100/($total_amount) as point_region ,
			u.province_id,
			rc.name
			FROM
			".DB_PREFIX."deal dl
			left join ".DB_PREFIX."user u on dl.user_id = u.id
  			left join ".DB_PREFIX."region_conf rc on u.province_id = rc.id
			WHERE
			(
				dl.deal_status > 1
				AND dl.deal_status != 3
			)
			and 
			(
				u.province_id > 0
		...
		$this->assign ('region_pie',$region_pie);	

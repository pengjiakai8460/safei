...
class ReferralsAction extends CommonAction{
	public function index()
	{
		...
		if($rel_user_name!=""){
			$condtion .= " and u.pid in(SELECT id FROM ".DB_PREFIX."user WHERE user_name='".$rel_user_name."') " ;
		}
		
		if($user_name!=""){
			$condtion .= " and u.user_name='".$user_name."' " ;
		}
		
		
		$sql_count = "SELECT count(r.id) FROM ".DB_PREFIX."referrals r LEFT JOIN ".DB_PREFIX."user u ON u.id =r.user_id WHERE 1=1 $condtion ";
		...
		if($count>0){
			$sql = "SELECT r.*,r.l_key +1 as l_key,dlr.true_interest_money,dlr.true_self_money,dlr.repay_date FROM ".DB_PREFIX."referrals r " .

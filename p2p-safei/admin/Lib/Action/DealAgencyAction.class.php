...
class DealAgencyAction extends CommonAction{
	...
	private function getUserList($user_type=0,$is_delete = 0,$map){
		...
		{
			if(intval($_REQUEST['is_mohu'])==0)
				$map['_string'] = " email_encrypt  = AES_ENCRYPT('".trim($_REQUEST['email'])."','".AES_DECRYPT_KEY."')";
			else
		...
		{
			if(intval($_REQUEST['is_mohu'])==0)
				$map['_string'] = " mobile_encrypt  = AES_ENCRYPT('".trim($_REQUEST['mobile'])."','".AES_DECRYPT_KEY."')";
			else
	...
	public function insert() {
		...
		if($_REQUEST['user_pwd']==""){
			unset($_REQUEST['user_pwd']);
		}
		// 更新数据
		$log_info = strim($_REQUEST['user_pwd']);
		$_REQUEST["user_type"] = 2;
	...
	public function edit() {		
		...
		$vo = get_user_info("*","id=".$id." and user_type = 2 ");
		$this->assign ( 'vo', $vo );		
	...
	public function update() {
		...
		$log_info = M("User")->where("id=".intval($_REQUEST['id']))->getField("user_name");
		//开始验证有效性

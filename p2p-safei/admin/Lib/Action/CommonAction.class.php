...
class CommonAction extends AuthAction{
	 ...
	 * @throws ThinkExecption
     +----------------------------------------------------------
	 */
	protected function _list($model, $map, $sortBy = '', $asc = false,$filed='*') {
		//排序字段 默认为主键名
		...
		if ($count > 0) {
			...
			if(strpos($order, "AES_DECRYPT") !==false){
				$order  =  $order ;
			}
			else{
				$order  = "`" . $order . "` ";
			...
			$voList = $model->field($filed)->where($map)->order( $order . $sort)->limit($p->firstRow . ',' . $p->listRows)->findAll ( );
...
//			echo $model->getlastsql();
	...
	{		
        ...
        $upload->allowExts  =  FilterAllowExt(); /* 配置于config */        
       
        $dir_name = to_date(TIME_UTC,"Ym");
	    ...
	    $dir_name = $dir_name."/".to_date(TIME_UTC,"d");
	    if (!is_dir(APP_ROOT_PATH."public/attachment/".$dir_name)) { 
	             @mkdir(APP_ROOT_PATH."public/attachment/".$dir_name);
	             @chmod(APP_ROOT_PATH."public/attachment/".$dir_name, 0777);
	        }
	     
	    $dir_name = $dir_name."/".to_date(TIME_UTC,"H");
	    if (!is_dir(APP_ROOT_PATH."public/attachment/".$dir_name)) { 
	             @mkdir(APP_ROOT_PATH."public/attachment/".$dir_name);
	             @chmod(APP_ROOT_PATH."public/attachment/".$dir_name, 0777);
	        }
...
//					Image::thumb($file_name,$big_file_name,'',$big_width,$big_height);
	...
	{	    
        ...
        if(isset($ext_arr[strim($_REQUEST['dir'])])){
            $upload->allowExts  =  $ext_arr[strim($_REQUEST['dir'])]; 
        }
        else
            $upload->allowExts  =  FilterAllowExt(); /* 配置于config */         
	...
	public function get_jx_json_pie($info,$type='',$pieinfotal) {
		$year =array_keys($info);
		...
		$color = array('#AABBCC','#FF0000','#440000','#FF368D','#117799','#152846','#ABCDEF','#123231','#44444','#228866','#AA22AA','#22CC33','#AC23CA','#FFAA11','#EFDE22','#DDEE55');
		$count = 5;
		$price_array=array();
		...
		foreach($price as $k1=>$v1){
			$i++;
			$price_array[$k1] = new pie_value(intval($v1['val']), "");
		    $price_array[$k1]->set_colour( $color[$i] );
		    $percent = number_format(intval($v1['val'])*100/intval($pieinfotal),1);
		    //$str = ($v1['val'])."\n ".$v1['name']." \n".$percent.'%';
		    $str = $v1['name']."($percent%)";
		    $price_array[$k1]->set_label($str, $color, 22 );
		}
		
		$pie->set_values($price_array);
	...
	public function get_jx_json_all_pie($value,$array,$pieinfotal){
		$data=array();
		foreach($array as $k=>$v){
			foreach($v as $k1=>$v1){
				if(isset($v1[3])&&!empty($v1[3])){
				...
				$data[$k][$k1]['pie'] = $this->get_jx_json_pie($value,$type_y,$pieinfotal);
				$data[$k][$k1]['title']=$v1[2];
			}

...
class InterestrateAction extends CommonAction{
	public function index()
	{
		$ecv_type = M("InterestrateType")->where("id=".intval($_REQUEST['ecv_type_id']))->find();
		if(!$ecv_type)
		{
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				...
				if ($list!==false) {
					M("InterestrateType")->where("id=".$ecv_type_id)->setField("gen_count",M("Interestrate")->where("ecv_type_id=".$ecv_type_id)->count());

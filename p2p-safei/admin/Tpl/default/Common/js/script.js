...
$(document).ready(function(){
	...
	//最近一年
    $("#submit_date_365").bind("click",function(){                   
        $("#q_start_time").val(dec_date(365));       
        $("#q_end_time").val(dec_date(0));  
        $('#search_form').submit();
    }); 
...
function bindKdedior(DsMode,FtMode,RdMode){
    ...
    var editor = K.create('textarea.ketext', {
        ...
        items : [
			...
			'flash', 'media', 'table', 'hr', 'emoticons', 'link', 'unlink','baidumap'
		]
    });   
...
function set_new(id,domobj)
{
		$.ajax({ 
				url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=set_new&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					...
					if(obj.data==1)
					{
						$(domobj).html(LANG['IS_NEW_1']);
					}
					else if(obj.data==0)
					{
						$(domobj).html(LANG['IS_NEW_0']);
					}
					else if(obj.data=='')
					{
...
function checkidcrad(card,realname){
	$.ajax({
		url:ROOT+"?m=Public&a=checkIdCard&card="+card+"&realname="+realname,
		dataType:"json",

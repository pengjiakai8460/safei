...
//				alert(ajaxobj.responseText);
			...
			$.ajax({
				...
				success:function(result){
					if(result.status==0){
						...
						$.weeboxs.open(APP_ROOT+"/index.php?ctl=uc_center&act=bind_management", {contentType:'ajax',showButton:false,title:'完成绑定',width:460,height:150,type:'wee'});	
							new_window.location.href = result.jump;
	...
	$("#J_APP_DOWN a").click(function(){
		...
		$.weeboxs.open("<div class='tc'><img src='"+url+"'  /></div>",{
			contentType:'text',showButton:true, showCancel:false, showOk:true,title:'扫描下载手机客户端',width:360,type:'wee'
		});
		return false;
	});
	//weixin;
	$("#weixim").click(function(){
		var obj=$(this);
		var url = obj.attr("href");
		$.weeboxs.open("<div class='tc'><img src='"+url+"'  /></div>",{
			contentType:'text',showButton:true, showCancel:false, showOk:true,title:'扫描访问微信端',width:360,type:'wee'
		});
		return false;
	});
...
function sendPhoneCode(o, obj , i , url){
	...
	if (!$(o).hasClass('btn_disable')) {
		...
		get_verify_code(o,obj,url,function(){
...
function get_verify_code(o,obj,url,func)
{
	var user_mobile = $(obj).val();
	var ajaxurl = "";
	if(url==null)
		ajaxurl = APP_ROOT+"/index.php?ctl=ajax&act=get_verify_code";
	else
		ajaxurl = url;
...
function sendPhoneCode0(o, obj){
	if (!$(o).hasClass('btn_disable')) {
		$(o).addClass('btn_disable');
		get_verify_code(o,obj,APP_ROOT+"/index.php?ctl=ajax&act=get_paypwd_verify_code",function(){
...
function set_sort(type,module)
{
	var ajaxurl = APP_ROOT+"/index.php?ctl=ajax&act=set_sort&type="+type+"&module="+module;
	$.ajax({ 
		url: ajaxurl,
		success: function(text){
			location.reload();
...
function collect_deal(o,deal_id,func)
{
	...
	$.ajax({
		...
		success: function(obj){
			if(obj.open_win == 1)
			{
				$.weeboxs.open(obj.html, {contentType:'text',showButton:false,title:LANG['PLEASE_LOGIN_FIRST'],width:1060,type:'wee'});
			}
			else
			{
				if(func!=null)
...
{	
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=ajax_login", {boxid:'pop_user_login',contentType:'ajax',showButton:false,title:LANG['PLEASE_LOGIN_FIRST'],width:1060,type:'wee',onopen:function(){
		init_ui_checkbox();	
		init_ui_textbox();
	},onclose:func});
}
...
function idcheck(o){
   ...
   var bday=$("select[name='bday']");
	...
	if(idcheck_act)
		idcheck_act.abort();
	idcheck_act = $.ajax({
		url:"index.php?ctl=ajax&act=getIdCardinfo&card="+str,
		dataType:"json",
		success:function(result){
			if(result['status']==1){
				if (result['sex'] == "男") 
			...
			$("select[name='sex']").ui_select({refresh:true});
		}
	});
...
function do_weixin_login()
{
	if(open_weixin_login ==1)
	{
		$.ajax({
			url:APP_ROOT+"/index.php?ctl=ajax&act=do_weixin_login",
			type:"POST",
...
$.fn.weixin_login = function(){
	$(this).live('click',function(){
		open_weixin_login=1;
		$.weeboxs.open(APP_ROOT+"/index.php?ctl=user&act=wx_login", {boxid:'pop_user_login',contentType:'ajax',showButton:false, showCancel:false, showOk:false,title:'微信登录',width:270,type:'wee',onopen:function(){

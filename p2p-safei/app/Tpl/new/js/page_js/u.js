...
jQuery(function(){
	...
	$("#Jcarry_From").submit(function(){
		...
		if($("select[name='region_lv2']").val()=="0" || $("select[name='region_lv3']").val()=="0" || $("select[name='region_lv4']").val()=="0" ){
			Jcarry_From_Lock = false;
			$.showErr("请填写完整的开户行所在地",function(){
				$("select[name='region_lv2']").focus();
			});
			return false;
		}
		if($.trim($("#Jbank_bankzone").val())==""){
			Jcarry_From_Lock = false;
			$.showErr("请填写开户行网点",function(){
...
function setCarryResult(){
	var carry_amount = 0;
	var total_amount =  parseFloat($("#Jcarry_totalAmount").val()) - parseFloat($("#Jcarry_nmc_amount").val());
	...
	$("#Jcarry_acount_balance").html(foramtmoney(acount_balance + parseFloat($("#Jcarry_nmc_amount").val()),2)+" 元");
}

...
class transferModule extends SiteBaseModule
{
	public function index(){
		$field = es_cookie::get("transfer_sort_field"); 
		$field_sort = es_cookie::get("transfer_sort_type"); 
		...
		switch($field){
			...
			case "remain_time":
				...
				break;
			case "t_user_id":
				$sfield = "dlt.t_user_id";
				break;
			case "month":
				$sfield = "dlt.transfer_number";
				break;
			default:
				$sfield = "";
		}
		...
		if(es_cookie::get("transfer_sort_field")=="ulevel"){
			$union_sql = ' LEFT join '.DB_PREFIX.'user u ON d.user_id = u.id ';
			$extfield = ",u.level_id ";
		}
		
		if($is_company > 0){
            $condition .= " AND d.user_id in ( SELECT id FROM ".DB_PREFIX."user WHERE user_type =".($is_company - 1)." )";
        }
        ...
        foreach($user_type_urls as $k=>$v){
            ...
            $user_type_urls[$k]['url'] = url("index","transfer#index",$tmp_args);
        }

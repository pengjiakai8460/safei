...
class licaiModule extends SiteBaseModule
{
	public function index()
	{
		//本周畅销
		$week_bestseller_list = get_licai_list("re_type=3 and status=1","sort DESC",1);
		$GLOBALS['tmpl']->assign("week_bestseller_list",$week_bestseller_list['list']);

		//限时抢购
		$limit_rush_list = get_licai_list("re_type=4 and status=1","sort DESC",1);
		$GLOBALS['tmpl']->assign("limit_rush_list",$limit_rush_list['list']);

		//新品上架
		$news_list = get_licai_list("re_type=1 and status=1","sort DESC",1);
		$GLOBALS['tmpl']->assign("news_list",$news_list['list']);
		...
		$GLOBALS['tmpl']->assign("special_list",$special_list);
		
		//余额宝
		$balance_list = get_licai_list("type = 0 and status=1 ","sort DESC,id DESC",5);
		$GLOBALS['tmpl']->assign("balance_list",$balance_list['list']);
		
		//定存宝
		$deposit_list = get_licai_list("type = 1 and status=1 ","sort DESC,id DESC",3);
		$GLOBALS['tmpl']->assign("deposit_list",$deposit_list['list']);
		...
		$licai['url']=url("collocation#RegisterCat",array('user_id'=>$GLOBALS['user_info']['id']));
		 
		$GLOBALS['tmpl']->assign('licai',$licai);
		
  		$GLOBALS['tmpl']->display("licai/licai_index.html");
	}	
	...
	public function deals(){
		...
		$p  =  $page->show("licai#deals",$filter_parms);
		...
		foreach($money_arr as $k=>$v){
			...
			$money_url[$k]['url'] = url("index","licai#deals",$tmp_filter); 
		}
		...
		foreach($rate_arr as $k=>$v){
			...
			$rate_url[$k]['url'] = url("index","licai#deals",$tmp_filter);
		}
		...
		foreach($orderby_arr as $k=>$v){
			...
			$orderby_url[$k]['url'] = url("index","licai#deals",$tmp_filter);
		}
	...
	public function uc_published_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_published_lc.html");
	...
	public function uc_record_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_record_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	// 发起的理财---理财管理
	public function uc_u_buyed_deal_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_u_buyed_deal_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	...
	public function uc_expire_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_expire_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	...
	public function uc_redeem_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_redeem_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	// 我购买的理财
	public function uc_buyed_lc(){
		...
		foreach($list as $k=>$v)
		{
			...
			$redempte_money = $GLOBALS["db"]->getOne("select sum(lcr.money) from ".DB_PREFIX."licai_redempte lcr left join ".DB_PREFIX."licai_order lco on lco.id=lcr.order_id where lco.licai_id = ".$v["licai_id"]." and lcr.status = 0 and lcr.user_id =".$GLOBALS["user_info"]["id"]." and lcr.order_id = ".$v['id']);
			
			if($v["have_money"] <= floatval($redempte_money))
			{
				$list[$k]["money_over"] = 1 ;
			}
			else
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_buyed_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	// 我购买的理财详情
	public function uc_buyed_deal_lc(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_buyed_deal_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	// 申请赎回
	public function uc_redeem(){
		...
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_redeem.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	//理财申请
	function uc_redeem_add()
	{
		...
		$result["jump"] = "";
		if(md5(FW_DESPWD($paypassword))!=$GLOBALS['user_info']['paypassword']){
			$result["status"] = 0;
			$result["info"] = "支付密码错误";
			ajax_return($result);	
		}
	...
	public function uc_deal_lc(){
		...
		{
			$list = $GLOBALS["db"]->getAll("select * from ".DB_PREFIX."licai_history where licai_id=".$id." and history_date <= '".to_date(TIME_UTC,"Y-m-d")."' order by history_date desc ");
			foreach($list as $k => $v)
			{
	...
	function uc_yeb_lc()
	{
		...
		if(to_timespan($vo["end_interest_date"])>=to_timespan(to_date(TIME_UTC,"Y-m-d")))
		{
			$vo["end_interest_date"] = to_date(TIME_UTC,"Y-m-d");
		}
		...
		//$GLOBALS['tmpl']->display("licai/licai_uc_published_lc.html");
		$GLOBALS['tmpl']->assign("inc_file","licai/licai_uc_yeb_lc.html");
		$GLOBALS['tmpl']->display("page/uc.html");
	}
	//发起理财
	...
	public function contract(){
		...
		$i_user_id = 0;
		$GLOBALS["tmpl"]->assign("type",0);
		$this->common_contract($id,$user_id,$i_user_id);
	}
	
	//电子合同 发布方
	public function i_contract(){
		$id = intval($_REQUEST['id']);
		if($id == 0){
			showErr("操作失败！");
		}
		$i_user_id = $GLOBALS["user_info"]["id"];
		$user_id = 0;
		$GLOBALS["tmpl"]->assign("type",1);
		$this->common_contract($id,$user_id,$i_user_id);
	}
	
	public function common_contract($id,$user_id=0,$i_user_id=0)
	{
		if($user_id == 0)
		{
			$licai_order = $GLOBALS['db']->getRow("select lo.*,lc.user_id as invest_user_id,lc.contract_id,lc.name FROM ".DB_PREFIX."licai_order lo left join ".DB_PREFIX."licai lc on lo.licai_id = lc.id WHERE lo.id=".$id." and lc.user_id=".$i_user_id." ORDER BY lo.create_time ASC");
		}
		elseif($i_user_id == 0)
		{
			$licai_order = $GLOBALS['db']->getRow("select lo.*,lc.user_id as invest_user_id,lc.contract_id,lc.name FROM ".DB_PREFIX."licai_order lo left join ".DB_PREFIX."licai lc on lo.licai_id = lc.id WHERE lo.id=".$id." and lo.user_id=".$user_id." ORDER BY lo.create_time ASC");
		}
		
		if(!$licai_order){
			showErr("操作失败！");
		}
		//$licai_order = licai_item_format($licai_order);
		
		$licai_order["money_format"] = format_price($licai_order["money"]);
		$licai_order["breach_rate_foramt"] = number_format($licai_order["breach_rate"],2)."%";
		$licai_order["interest_rate_foramt"] = number_format($licai_order["interest_rate"],2)."%";
		
		$GLOBALS['tmpl']->assign('vo',$licai_order);
		
		$u_info = get_user("*",$licai_order['user_id']);
		
		$invite_info = get_user("*",$licai_order['invest_user_id']);
		...
		$contract = $GLOBALS['tmpl']->fetch("str:".get_contract($licai_order['contract_id']));
		
		$GLOBALS['tmpl']->assign('contract',$contract);
		
		$GLOBALS['tmpl']->display("licai/licai_contract.html");	
	}
	...
	public function dcontract(){
		...
		$user_id = $GLOBALS["user_info"]["id"];
		$i_user_id = 0;
		$this->common_dcontract($id,$user_id,$i_user_id);
	}
	
	//电子合同 发布方
	public function i_dcontract(){
		$id = intval($_REQUEST['id']);
		if($id == 0){
			showErr("操作失败！");
		}
	...
	public function common_dcontract($id,$user_id,$i_user_id)
	{
		if($user_id == 0)
		{
			$licai_order = $GLOBALS['db']->getRow("select lo.*,lc.user_id as invest_user_id,lc.contract_id,lc.name FROM ".DB_PREFIX."licai_order lo left join ".DB_PREFIX."licai lc on lo.licai_id = lc.id WHERE lo.id=".$id." and lc.user_id=".$i_user_id." ORDER BY lo.create_time ASC");
    	...
    	$word = new word(); 
   		$word->start(); 
   		$wordname = "理财协议.doc"; 
   		echo  $GLOBALS['tmpl']->fetch("licai/licai_contract.html");
   		$word->save($wordname); 

	...
	public function export_csv($page = 1)
	{
		...
		foreach($list as $k=>$v){
			if($acname =="index"){
				...
				{
					$list[$k]['time'] = $list[$k]['tras_create_time_format']."-".$list[$k]['final_repay_time_format'];
				}else{
					$list[$k]['time'] = $list[$k]['final_repay_time_format'];
				}
			}elseif ($acname =="buys"){
				$list[$k]['time'] = $list[$k]['transfer_time_format']." - ".$list[$k]['final_repay_time_format'];
			}
			$list[$k]['daizong'] = $list[$k]['how_much_month']."/".$list[$k]['repay_time'];
		...
		if($list)
		{
			...
			$repay_value = array('time'=>'""','name'=>'""','daizong'=>'""','left_benjin_format'=>'""','left_lixi_format'=>'""','transfer_amount_format'=>'""','tras_status_format'=>'""');
	
			$content = "";
			$contentss = iconv("utf-8","gbk","转让时间-到期时间,项目,待还/总期数,待收本金,待收利息,转让价格,状态");
			$content  .= $contentss . "\n";
			foreach($list as $k=>$v)
			{
				$repay_value = array();
				$repay_value['time'] = iconv('utf-8','gbk','"' . $list[$k]['time'] . '"');
				$repay_value['name'] = iconv('utf-8','gbk','"' . $list[$k]['name'] . '"');
				$repay_value['daizong'] = iconv('utf-8','gbk','"' . $list[$k]['daizong'] . '"');
				$repay_value['left_benjin_format'] = iconv('utf-8','gbk','"' . $list[$k]['left_benjin_format'] . '"');
				$repay_value['left_lixi_format'] = iconv('utf-8','gbk','"' . $v['left_lixi_format'] . '"');
				$repay_value['transfer_amount_format'] = iconv('utf-8','gbk','"' . $list[$k]['transfer_amount_format'] . '"');
				$repay_value['tras_status_format'] = iconv('utf-8','gbk','"' . $v['tras_status_format'] . '"');
				$content .= implode(",", $repay_value) . "\n";
			}
	
			header("Content-Disposition: attachment; filename=transfer_".$acname.".csv");
			echo $content;
		}
		else
		{

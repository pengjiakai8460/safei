...
class borrowModule extends SiteBaseModule
{
    ...
    function quota_submit(){
		...
		$typeid = $deal_quota['type_id'];
		$type_info = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."deal_loan_type WHERE id=".$typeid);
		
		if($type_info['costsetting'] && $GLOBALS['user_info']['vip_id']>0 ){
			$vo_list = explode("\n",$type_info['costsetting']);
			foreach($vo_list as $k=>$v){
				$vo_list[$k] = explode("|",$v);
				if($vo_list[$k]['0'] == $GLOBALS['user_info']['vip_id'] ){
					if($vo_list[$k]['1']>0||$vo_list[$k]['2']>0||$vo_list[$k]['3']>0||$vo_list[$k]['4']>0||$vo_list[$k]['5']>0||$vo_list[$k]['6']>0){
						$load_mfee = $vo_list[$k]['2'];
						$services_fee = $vo_list[$k]['1'];
						$GLOBALS['tmpl']->assign("services_fee",$services_fee);
					}
				}
			}
		}else{
			if($type_info['levelsetting']){
				...
				foreach($vol_list as $ks=>$vs){
					...
					$vol_list[$ks]['name'] = $GLOBALS['db']->getOne("SELECT name FROM ".DB_PREFIX."user_level WHERE id=".$vol_list[$ks]['0']);
					if($vol_list[$ks]['0'] == $GLOBALS['user_info']['level_id'] ){
						if($vol_list[$ks]['1']>0||$vol_list[$ks]['2']>0||$vol_list[$ks]['3']>0||$vol_list[$ks]['4']>0||$vol_list[$ks]['5']>0||$vol_list[$ks]['6']>0){
							$load_mfee = $vol_list[$ks]['2'];
							$services_fee = $vol_list[$ks]['1'];
						}
					}	
				}
				$GLOBALS['tmpl']->assign('levelsetting_list',$vol_list);
			}
		}
    ...
    function savedealquota(){
    	...
    	$data['deal_status'] = 1;
    	$data['is_mortgage'] = intval($_REQUEST['is_mortgage']);
    	$data['mortgage_desc'] = strim($_REQUEST['mortgage_desc']);
    	unset($data['id']);
   ...
   //撤销审核 
    function reback(){
    	...
    	if($GLOBALS['db']->affected_rows()>0){
    		...
    		showErr("撤销失败，请检测借款是否存在，或者是否已经审核通过",1);
    	}
    }

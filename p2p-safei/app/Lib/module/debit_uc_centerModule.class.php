...
class debit_uc_centerModule extends SiteBaseModule
{
	...
	public function init_user(){
		...
		$GLOBALS['tmpl']->assign('can_use_quota',intval($can_use_quota));
	...
	public function order()
	{
		...
		$result = get_deal_list($limit,0,"(deal_status in (0,1,2,3,4,5) or publish_wait in (1,2,3) ) AND user_id=".$user_id,"id DESC",'','',true);
		$deal_ids = array();
			...
			foreach($result['list'] as $k=>$v){
				...
				switch($v["deal_status"])
				{
					case "0":
						if($v["is_delete"]==3)
						{
							$result['list'][$k]['deal_status'] = "订单已取消";break;
						}
						else
						{
	...
	public function order_detail()
	{
		...
		$first_relief = 0;
		//首单 减钱
		$deal_admin = $GLOBALS["db"]->getOne("select admin_id from ".DB_PREFIX."deal where id = ".$deal["id"]);
		if($deal_admin)
		{
		...
		foreach($loan_list as $k => $v)
		{
			if($v["has_repay"]==1)
			{
				$loan_list[$k]["month_repay_money_format"] = format_price($v["month_has_repay_money"]);
				if($first_relief > 0 && $v["l_key"] == 0 )
				{
					$loan_list[$k]["month_repay_money_format"] = format_price($v["month_has_repay_money"] )."-(".format_price($first_relief)."红包)";
				}
				$left_repay_count --;
			}
			elseif($v["l_key"] == 0 && $v["has_repay"] == 0 && $first_relief >0 )
			{
				$loan_list[$k]["month_repay_money_format"] = format_price($v["month_repay_money"]);
				$loan_list[$k]["month_repay_money_format"] = format_price($v["month_repay_money"] )."-(".format_price($first_relief)."红包)";
				$loan_list[$k]["month_need_all_repay_money_format"] = format_price($v["month_need_all_repay_money"] - $first_relief);
			}
		}
		
		//$loan_list["last_money"] = $loan_list["compensate_fee"] + $loan_list[""] - $loan_list["repay_money"];
		
		$deal["left_repay_count"] = $left_repay_count;
		
		$GLOBALS['tmpl']->assign("loan_list",$loan_list);
		$GLOBALS['tmpl']->assign("first_relief",$first_relief);
		$GLOBALS['tmpl']->assign("deal",$deal);
		/*提前还款显示*/
	...
	public function save()
	{
		...
		if($res['status'] == 1)
		{
			...
			$user_info = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where id = '".intval($s_user_info['id'])."'");

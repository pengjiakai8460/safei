...
class uc_orderModule extends SiteBaseModule
{
	...
	public function view()
	{
		...
		{
			$order_info['region_lv1'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."region_conf where id = ".$order_info['region_lv1']);
			$order_info['region_lv2'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."region_conf where id = ".$order_info['region_lv2']);
			$order_info['region_lv3'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."region_conf where id = ".$order_info['region_lv3']);
			$order_info['region_lv4'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."region_conf where id = ".$order_info['region_lv4']);		
	...
	public function modify()
	{
		...
		$cart_list = $GLOBALS['db']->getAll("select doi.*,d.icon from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id = d.id where doi.order_id = ".$order_info['id']);
				...
				{
					//输出配送方式
					$consignee_id = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."user_address where user_id = ".$GLOBALS['user_info']['id']);

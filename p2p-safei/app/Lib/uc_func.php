...
// +----------------------------------------------------------------------
	...
	{
		$user_id = intval($user_id);
		$sql = "select e.*,et.name,et.gen_count,et.send_type,et.exchange_score,et.exchange_limit,et.exchange_sn from ".DB_PREFIX."ecv as e left join ".DB_PREFIX."ecv_type as et on e.ecv_type_id = et.id where e.user_id = ".$user_id." order by e.id desc limit ".$limit;
...
//					}
	...
	function getUcSaveCarry2($amount,$region_lv1,$region_lv2,$region_lv3,$region_lv4,$bankzone,$paypassword,$bid){
		$status = array('status'=>0,'show_err'=>'');
		...
		if($GLOBALS['user_info']['id'] > 0){
			...
			if((floatval($fee) + floatval($data['money'])) <= get_user_info("AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."')-nmc_amount","id=".intval($GLOBALS['user_info']['id']),"ONE")){
				...
				$data['bank_id'] = $user_bank['bank_id'];
				$data['real_name'] = $user_bank['real_name'];
				$data['region_lv1'] = $region_lv1;
				$data['region_lv2'] = $region_lv2;
				$data['region_lv3'] = $region_lv3;
				$data['region_lv4'] = $region_lv4;
				$data['bankzone'] = trim($bankzone);
				$data['bankcard'] = trim($user_bank['bankcard']);

...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // session 过期
        if(TIME_UTC - $secode['verify_time'] > $this->expire) {
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            ...
            for ($i = 0; $i<$this->length; $i++) {
				...
				if($this->fontSize==12)
					$codeNX  = $this->fontSize*($i+0.1);
				else
                	$codeNX  += mt_rand($this->fontSize*0.9, $this->fontSize*1);
                imagettftext($this->_image, $this->fontSize, mt_rand(-30, 30), $codeNX, ($this->fontSize==12 ? $this->fontSize*1.1 : $this->fontSize*1.6), $this->_color, $this->fontttf, $code[$i]);
            }
        }
      ...
      // 保存验证码
        ...
        $secode['verify_time'] = TIME_UTC;  // 验证码创建时间

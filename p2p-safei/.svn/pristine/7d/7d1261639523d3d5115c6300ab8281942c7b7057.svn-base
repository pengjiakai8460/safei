...
class DealQuotaSubmitAction extends CommonAction{
	...
	public function update(){
		...
		if($point>0){
			$msg="授信额度申请成功，更改信用额度".$point;
			require_once APP_ROOT_PATH."system/libs/user.php";
			modify_account(array('quota'=>$point),$data['user_id'],$msg,8);
		...
		if ($list > 0) {
			...
			if($data['status']==1){
				...
				//短信通知
				if(app_conf("SMS_ON")==1 && app_conf('QUOTA_SUCCESS_SMS')==1)
				{
				...
				if(app_conf("WEIXIN_MSG")==1){
					...
					{
						$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."weixin_tmpl where template_id_short='OPENTM207582660' ");
						$today = to_date(TIME_UTC,"Y-m-d");
						$weixin_data['first'] = array('value'=>$user_info['user_name'].'您的信用额度申请','color'=>'#173177');
						$weixin_data['keyword1']=array('value'=>$vo['money'],'color'=>'#173177');
						$weixin_data['keyword2']=array('value'=>$today,'color'=>'#173177');
				...
				//短信通知
				if(app_conf("SMS_ON")==1 && app_conf('QUOTA_FAILED_SMS')==1)
				{

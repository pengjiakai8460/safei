...
class uc_moneyModule extends SiteBaseModule
{
	...
	public function index()
	{
		...
		if($t=="point"){
			...
			$title_arrays = load_auto_cache("cache_money_type",array("class"=>"point"));
		}
		elseif($t=="lock_money"){
			/*$title_arrays = array(
				"100" => "全部",
		...
		elseif($t=="score"){
			...
			$title_arrays = load_auto_cache("cache_money_type",array("class"=>"nmc_amount"));
		}
		else{
			/*$title_arrays = array(
				"100" => "全部",
	...
	public function incharge()
	{
		...
		if($GLOBALS['db']->getOne("SELECT user_id FROM ".DB_PREFIX."user_credit_file WHERE user_id=".$GLOBALS['user_info']['id']." AND passed=1")==false){
			$incharge_html="<div>平台为了保证您的资金安全，请先进行实名认证，认证通过后方可进行充值，<a href='index.php?ctl=borrow&act=applyamount' style='color: #0659b8;'>立即认证>></a></div>";
			$GLOBALS['tmpl']->assign("incharge_html",$incharge_html);
		}
		...
		//同卡进出
		$user_bankcard = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_bank WHERE user_id = ".$GLOBALS['user_info']['id']);
		if(!empty($user_bankcard))
		{
			$user_bankcard['idno'] = $GLOBALS['db']->getOne("select idno from ".DB_PREFIX."user WHERE id = ".$GLOBALS['user_info']['id']);
			$has_bank = 1;
		}else{
			$has_bank = 0;
		}
		$GLOBALS['tmpl']->assign("has_bank",$has_bank);
		$GLOBALS['tmpl']->assign("user_bankcard",$user_bankcard);
	...
	*/
	...
	public function carry()
	{
		...
		if($bid==0){
			showErr("请先选择提现银行卡",0,url("index","uc_money#bank"));
		...
		if(!$user_bank){
			showErr("所选银行卡不存在",0,url("index","uc_money#bank"));
		}
	...
	function savecarry(){
		...
		if($GLOBALS['user_info']['id'] > 0){
			...
			$region_lv1 = floatval($_REQUEST['region_lv1']);
			$region_lv2 = floatval($_REQUEST['region_lv2']);
			$region_lv3 = floatval($_REQUEST['region_lv3']);
			$region_lv4 = floatval($_REQUEST['region_lv4']);
			$bankzone = $_REQUEST['bankzone'];
			
			$status = getUcSaveCarry2($amount,$region_lv1,$region_lv2,$region_lv3,$region_lv4,$bankzone,$paypassword,$bid);

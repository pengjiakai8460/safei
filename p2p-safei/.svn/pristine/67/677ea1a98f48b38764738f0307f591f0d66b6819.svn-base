...
class uc_score_exchangeModule extends SiteBaseModule {
     ...
     public function index(){
		...
		$userinfo=get_user_info("*","id='".$GLOBALS['user_info']['id']."' and vip_state='1' ");
...
//		$load_total = $GLOBALS['db']->getOne("select sum(money) FROM ".DB_PREFIX."deal_load WHERE create_date>'$start_time' and user_id='$user_id' ");
		...
		if(TIME_UTC < $userinfo['vip_end_time']){
			$is_vip_intime = 1;
			$GLOBALS['tmpl']->assign('is_vip_intime',$is_vip_intime);
		}
	...
	public function cash_save(){
		...
		$userinfo=get_user_info("*","id='".$GLOBALS['user_info']['id']."' ");

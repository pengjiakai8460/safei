<?php
class credit_agenciesModule extends SiteBaseModule {
	private $is_ajax ; 
    ...
    function index() {
    	...
    	if($type=="credit_contact" || $type=="credit_residence"){
    		//地区列表
				$work =  $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."user_work where user_id =".$authorized_info['id']);
    	...
    	if($this->is_ajax==1)
    		showSuccess($GLOBALS['tmpl']->fetch("authorized/authorized_credit_box.html"),$this->is_ajax);
    	else
    		$GLOBALS['tmpl']->display("authorized/authorized_credit_box.html");
    }
    ...
    function credit_save(){
    	...
    	if($type == "credit_identificationscanning"){
    		...
    		if(get_user_info("count(*)","idno_encrypt = ".$u_c_data['idno_encrypt']." and id <> ".intval($authorized_info['id']),"ONE")>0){

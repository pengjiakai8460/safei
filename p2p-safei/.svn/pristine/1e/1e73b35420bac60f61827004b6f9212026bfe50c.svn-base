...
class loantype_2 implements loantype{
		...
		foreach($load_users as $k=>$v){
			...
			if($idx+1 == $true_repay_time){
                ...
                if($k+1 == count($load_users)){
                    $item['repay_manage_money'] = $deal['all_manage_money'] -  round($deal['all_manage_money']/ $deal['buy_count'],2) * ($deal['buy_count'] - 1);
            ...
            if($deal['repay_time_type'] == 1)
                ...
                $item['manage_money'] = $item['money'] * $deal["repay_time"] * floatval($deal["user_loan_manage_fee"])*0.01 / 30;
	...
	function inrepay_repay($loaninfo,$k,$time_utc=0){
        ...
        $return["true_mortgage_fee"] = ($loaninfo['deal']['all_mortgage_fee']/$all_repay_time) * $true_m;

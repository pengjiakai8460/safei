...
define("ACCOUNT_NO_VERIFY_ERROR",3); //帐户未激活
	 ...
	 * @param integer $type  0结存，1充值，2投标成功，3招标成功，4偿还本息，5回收本息，6提前还款，7提前回收，8申请提现，9提现手续费，10借款管理费，11逾期罚息，12逾期管理费，13人工充值，14借款服务费，15出售债权，16购买债权，17债权转让管理费，18开户奖励，19流标还返，20投标管理费，21投标逾期收入，22兑换，23邀请返利，24投标返利
	 */
		...
		*/
		...
		else
		{
			...
			if($is_use_pass) //未整合，则直接成功
			{
				...
				*/
				es_session::set("authorized_info",$user_data);
				$GLOBALS['authorized_info'] = $user_data;
				//检测勋章
				/*$medal_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."medal where is_effect = 1 and allow_check = 1");
				foreach($medal_list as $medal)
				{

...
class LicaiRedempteAction extends CommonAction{
	...
	{	
		...
		foreach($result['list']  as $k => $v)
		{
			...
			$days = (to_timespan($v["create_date"]) - to_timespan($v["before_interest_date"]))/24/3600+1;
			if($days < 0)
			{
				$days = 0;
			}
	...
	{	
		...
		$condition = " and lcr.type = 1 and lcr.status in (0,1,2) ";
		...
		if($list)
		{
			...
			$order_value = array( 'id'=>'""', 'name'=>'""', 'type_format'=>'""','user_name'=>'""','have_money_format'=>'""','show_rate_format'=>'""','earn_money_format'=>'""','money_format'=>'""','fee_money_format'=>'""','create_date'=>'""','status_format'=>'""');
	    	if($page == 1)
	    	{	
			...
			foreach($list as $k=>$v)
			{
				...
				$order_value['have_money_format'] = '"' . iconv('utf-8','gbk',$v['have_money']) . '"';
				$order_value['money_format'] = '"' . iconv('utf-8','gbk',$v['money_format']) . '"';
				$order_value['show_rate_format'] = '"' . iconv('utf-8','gbk',$v['show_rate_format']) . '"';
				$order_value['fee_money_format'] = '"' . iconv('utf-8','gbk',$v['fee_money_format']) . '"';
				$order_value['earn_money_format'] = '"' . iconv('utf-8','gbk',$v['earn_money_format']) . '"';
	...
	{	
		...
		if($list)
		{
			...
			foreach($list as $k=>$v)
			{
				...
				$order_value['user_name'] = '"' . iconv('utf-8','gbk',$v['user_name']) . '"';
				$order_value['have_money_format'] = '"' . iconv('utf-8','gbk',$v['have_money']) . '"';
				$order_value['money_format'] = '"' . iconv('utf-8','gbk',$v['money']) . '"';
				$order_value['show_rate_format'] = '"' . iconv('utf-8','gbk',$v['show_rate_format']) . '"';
	...
	function foreach_pay($page = 1)
	{
		...
		/**/
		$condition = " and lcr.type = 1 and lcr.status = 0";
		...
		$count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."licai_redempte lcr 
		left join ".DB_PREFIX."licai_order lco on lcr.order_id = lco.id 
		left join ".DB_PREFIX."licai lc on lco.licai_id = lc.id where 1=1 ".$condition);

		if($count > 0){
			
			$list = $GLOBALS['db']->getAll("SELECT lcr.*,lco.user_name,lco.licai_id,lco.money as order_money, 
			lc.type as licai_type, lc.name,lco.begin_interest_date,
			lco.site_buy_fee,lco.redempte_money ,lco.money-lco.site_buy_fee-lco.redempte_money as have_money 
			FROM ".DB_PREFIX."licai_redempte lcr 
			left join ".DB_PREFIX."licai_order lco on lcr.order_id = lco.id 
		...
		if($list)
		{
			...
			foreach($list as $k => $v)
			{
				...
				$redempte_id = $v["id"];				
				$earn_money = $v['earn_money'];
				$fee = $v['fee'];
				$organiser_fee = $v["organiser_fee"];
	...
	//批量理财发放
	function foreach_pay_before($page = 1)
	{
		$pagesize = 10;
		set_time_limit(0);
		$limit = (($page - 1)*intval($pagesize)).",".(intval($pagesize));
		...
		/**/
		$condition = " and lcr.type = 0 and lcr.status = 0";

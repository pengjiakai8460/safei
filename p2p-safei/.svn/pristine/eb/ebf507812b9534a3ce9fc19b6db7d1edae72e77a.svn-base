...
function check_dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc = 0,$ecv_id=0,$learn_id = 0,$interestrate_id=0,$use_interestrate=0){	
	...
	if(intval($use_interestrate) == 0)
	{
		if(intval($ecv_id) > 0){
			...
			$root['ecv_money'] = $ecv['money'];
		}
	}
	elseif(intval($use_interestrate) > 0 )
	{
...
//		}
	...
	if (!empty($deal['ips_bill_no'])){
		//第三方托管标
		if (!empty($GLOBALS['user_info']['ips_acct_no'])){
			$result = GetIpsUserMoney($GLOBALS['user_info']['id'],$GLOBALS['user_info']['user_type']);
			if(floatval($result['pBalance']) < $root["bid_money"]){
				$root["status"] = 0;
				$root["show_err"] = "资金不足，请先充值";
				return $root;
			}
		}
	}
...
function dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc=0,$ecv_id=0,$learn_id=0,$interestrate_id=0,$use_interestrate=0){
	$root = check_dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc,$ecv_id,$learn_id,$interestrate_id,$use_interestrate);
	if ($root["status"] == 0){
		return $root;
	}
	...
	if($bid_money - floatval($root['ecv_money']) - floatval($root['learn_money']) > $GLOBALS['user_info']['money']){
	...
	$data['learn_money'] = $root['learn_money'];
	$data['learn_id'] = $root['learn_id'];
	...
	if($load_id > 0){
		...
		if($interestrate_id > 0){
			$GLOBALS['db']->query("UPDATE ".DB_PREFIX."interestrate SET use_count = use_count + 1 WHERE (begin_time =0 OR (begin_time > 0 AND begin_time < ".TIME_UTC.")) AND (end_time = 0 OR (end_time > 0  AND (end_time +24*3600 - 1) > ".TIME_UTC.")) AND (use_limit =0  OR (use_limit >0 AND use_limit - use_count > 0)) AND id=".$interestrate_id." AND ((user_id=".$GLOBALS['user_info']['id'].") or to_user_id = ".$GLOBALS['user_info']['id'].")");
		}
		...
		//更改资金记录
		if($bid_money - $ecv_money - $insertdata['learn_money'] > 0){
			
			$msg = '[<a href="'.$root['deal']['url'].'" target="_blank">'.$root['deal']['name'].'</a>]的投标,付款单号'.$load_id;
			require_once APP_ROOT_PATH."system/libs/user.php";
			modify_account(array('money'=>-($bid_money - $ecv_money - $insertdata['learn_money']),'lock_money'=>($bid_money - $ecv_money-$insertdata['learn_money'])),$GLOBALS['user_info']['id'],$msg,2);
		
		}
	
		$lottery_num = $bid_money / 5000;

			$GLOBALS['db']->query("update ".DB_PREFIX."user set lottery_num = lottery_num+".$lottery_num." WHERE id =".$data['user_id']);

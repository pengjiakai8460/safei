...
class authorizedModule extends SiteBaseModule
{
	static private function checkLogin(){
		...
		$authorized_info = get_user_info("*","id = ".$info["id"]);
		if(!$authorized_info|| $authorized_info['is_effect']==0 || $authorized_info['user_type']!=3){
	...
	public function login()
	{
		...
		{
			$GLOBALS['tmpl']->assign("page_title","授权服务机构登陆");
	...
	public function dologin()
	{
		...
		if($result['status'])
		{
		    if(!es_session::is_set("authorized_info"))	{
		        es_session::delete("user_info");
		        showErr($GLOBALS['lang']['USER_NOT_EXIST'],$ajax);
		    }
	...
	public function saveemail(){
		...
		$authorized_info = get_user_info("*","id =".$authorized_info["id"]);
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."user set email_encrypt = AES_ENCRYPT('".$email."','".AES_DECRYPT_KEY."'),verify = '',emailpassed = 1 where id = ".$GLOBALS['authorized_info']['id']);
	...
	function account(){
		...
		$authorized_info = get_user_info("*","user_type = 3 and id=".$authorized_info['id']);
	...
	function accountsave(){
		...
		$agency_info = get_user_info("*","id=".$authorized_info['id']);
	...
	public function save_idsno()
	{
		...
		if(get_user_info("count(*)","id =".$id,"ONE"))
		{	
			$user_info_re = array();

			$user_info_re['real_name_encrypt'] = "AES_DECRYPT('".$real_name."','".AES_DECRYPT_KEY."')";
			$user_info_re['idno_encrypt'] = "AES_DECRYPT('".$idno."','".AES_DECRYPT_KEY."')";
	...
	public function customer()
	{
		...
		$result = get_user_info("*","pid = ".$authorized_info['id']." order by id desc".$limit,"ALL");
		$count  = get_user_info("COUNT(*)","pid = ".$authorized_info['id'],"ONE");
	...
	public function customer_not()
	{
		...
		$result = get_user_info("*","referer_memo = ".$authorized_info['id']." and pid = 0 order by id desc".$limit,"ALL");
		$count  = get_user_info("count(*)","referer_memo = ".$authorized_info['id']." and pid = 0 ","ONE");
...
LEFT JOIN ".DB_PREFIX."user t_u ON t_u.id = t_user_id LEFT JOIN ".DB_PREFIX."user f_u ON f_u.id = dl.user_id where d.is_delete=0 and d.publish_wait = 0 AND d.deal_status in(4,5) and (f_u.pid='".$authorized_info["id"]."' or t_u.pid = '".$authorized_info["id"]."')";
	...
	public function refdetail(){
		...
		$authorized_info = $this->checkLogin();
    	$user_id = intval($_REQUEST['u_id']);
		$auth = get_user_info("*","id = $user_id and pid = ".$authorized_info["id"]);
		if(!$auth)
		{
			showErr("非法访问",0);
		}
	...
	public function invite()
	{
		...
		$referral_user = get_user_info("count(*)","pid = ".$GLOBALS['authorized_info']['id']." and is_effect=1 and is_delete=0","ONE");
		...
		if($GLOBALS['authorized_info']){
			$share_url_mobile = $share_url.$depart."r=".str_replace('+', '%2b', base64_encode($GLOBALS['authorized_info']['mobile']));
			$share_url_username = $share_url.$depart."r=".str_replace('+', '%2b', base64_encode($GLOBALS['authorized_info']['user_name']));
		}

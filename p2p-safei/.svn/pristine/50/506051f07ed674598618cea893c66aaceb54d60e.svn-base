...
class Phpwind_integrate implements integrate {
	...
	{			
        ...
        $es_user = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where user_name = '".$user_name."'");
        ...
        else
        {
        	...
        	if(!$es_user)
        	{
	        	...
	        	if($row&&$row['password']==md5($user_pwd))  //pw有该会员并且密码正确
	        	{
					...
					if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where email_encrypt = AES_ENCRYPT('".$row['email']."','".AES_DECRYPT_KEY."')")>0)  //会员邮箱已存在时邮箱留空
					{
							$email = ''; 
					}

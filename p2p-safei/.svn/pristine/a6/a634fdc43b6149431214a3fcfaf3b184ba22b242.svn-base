...
class manageagencyModule extends SiteBaseModule
{
	static private function checkLogin(){
		...
		$manageagency_info = get_user_info("*","id=".$manageagency_info['id']);
	...
	public function doregister()
	{
		...
		$pid = get_user_info("id","mobile_encrypt =AES_ENCRYPT('".$user_data['user_name']."','".AES_DECRYPT_KEY."') OR `user_name`='".$user_data['user_name']."'","ONE");
	...
	public function dologin()
	{
		...
		if($result['status'])
		{
			...
			if(!es_session::is_set("manageagency_info")){
	...
	public function bidverify()
	{
		...
		if($user_info['verify']!=''&&$user_info['verify'] == $verify)
		{
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."user set login_ip = '".CLIENT_IP."',login_time= ".TIME_UTC.",verify = '',emailpassed = 1,email_encrypt=AES_ENCRYPT(tmp_email,'".AES_DECRYPT_KEY."'),is_effect = 1 where id =".$user_info['id']);
	...
	function accountsave(){
		...
		$agency_info = get_user_info("*","id=".$manageagency_info['id']);
	 ...
	 */
	 ...
	 function pingzheng(){
		...
		require APP_ROOT_PATH."app/Lib/page.php";
	 	...
	 	$view_info = get_user_info("view_info","id=".$agency_id,"ONE");
	 ...
	 function add_pingzheng()
	 {
	 	...
	 	$old_view_infos =  get_user_info("view_info","id=".$agency_id,"ONE");
	...
	public function save_ips()
	{
		...
		//判断该实名是否存在
		if(get_user_info("count(*)","ips_acct_no = '.$ips_acct_no.' and id<> $id ","ONE") > 0 ){
			$data['status'] = 0;
			$data['info'] = "该IPS账户已被其他用户绑定，非本人请联系客服";
			ajax_return($data);
		}
				
		if(get_user_info("count(*)","id =".$id,"ONE") > 0)
		{	
			$user_info_re = array();
			$user_info_re['id'] = $id;
	...
	public function saveemail(){
		...
		$GLOBALS['manageagency_info']  = get_user_info("*","id = '".intval($GLOBALS['manageagency_info']['id'])."'");
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."user set email_encrypt = AES_ENCRYPT('".$email."','".AES_DECRYPT_KEY."'),emailpassed = 1 where id = ".$GLOBALS['manageagency_info']['id']);

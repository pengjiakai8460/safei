...
class LicaiAdvanceAction extends CommonAction{
	...
	public function update()
	{
		...
		$advance = $GLOBALS["db"]->getRow("select * from ".DB_PREFIX."licai_advance where id =".$data["id"]);
		
		$user_info = get_user_info("*","id = ".$advance["user_id"]);
					  
		if($user_info['money'] >= $redempte["advance_money"])
		{
			modify_account(array('money'=>'-'.$advance['advance_money']),$advance["user_id"],"垫付单收回,单号：".$advance["id"],49);
		}
		else
		{
			//错误提示

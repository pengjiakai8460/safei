	...
	public function load($param)
	{
		...
		$consignee_data = $GLOBALS['cache']->get($key);
			if($consignee_data === false)
			{
				...
				$region_lv1 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where pid = 0");  //一级地址
				...
				$region_lv2 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where pid = ".$consignee_info['region_lv1']);  //二级地址
				...
				$region_lv3 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where pid = ".$consignee_info['region_lv2']);  //三级地址
				...
				$region_lv4 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where pid = ".$consignee_info['region_lv3']);  //四级地址

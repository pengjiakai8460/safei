...
class SecurityAction extends CommonAction{
	public function index()
	{
		...
		if($begin_time > 0 || $end_time > 0){
			if($end_time==0)
			{
				$condition .= " and s.optime >= $begin_time ";
			}
			else
				$condition .= " and s.optime between  $begin_time and $end_time ";
		}
		...
		$sql_count = " SELECT count(*) FROM ".DB_PREFIX."security s left join ".DB_PREFIX."admin a on a.id=s.admin_id  WHERE $condition ";
		...
		if($rs_count > 0){
			...
			$sql_list =  " select s.* ,a.adm_name from ".DB_PREFIX."security s left join ".DB_PREFIX."admin a on a.id=s.admin_id  WHERE $condition ORDER BY $order $sort LIMIT ".$p->firstRow . ',' . $p->listRows;
	...
	public function insert()
	{
		...
		$data = M("Security")->create ();
		
		$money = $GLOBALS['db']->getOne("select sum(amount) from ".DB_PREFIX."security");
	...
	public function delete(){
		...
		if (isset ( $id )) {
			...
			//删除的验证
			$list = M("Security")->where ( $condition )->delete();
			if ($list!==false) {

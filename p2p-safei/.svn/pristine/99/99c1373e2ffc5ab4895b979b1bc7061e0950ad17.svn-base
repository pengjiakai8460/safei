...
class VipPrivilegeAction extends CommonAction {
	...
	private function getUserList($user_type=0,$is_delete = 0,$map){
		...
		$sql_str="select id,user_name,vip_id,vip_state,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,byear,bmonth,bday,customer_id from ".DB_PREFIX."user where user_type in(0,1) ";
		$sql_str.=" AND is_delete = $is_delete ";
		$sql_str .=$map;

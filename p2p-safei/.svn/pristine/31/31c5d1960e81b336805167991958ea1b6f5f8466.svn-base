...
class goods_informationModule extends SiteBaseModule
{
	...
	public function address(){
		...
		$sql = "SELECT is_delivery from ".DB_PREFIX."goods where id = ".$goods_id ;
		$is_delivery = $GLOBALS['db']->getOne($sql);
	...
	public function doexchange(){
		...
		//库存属性 star
		$attr_str = $attr_stocks['attr_str'];
		$goods_attr_scoresum = $attr_stocks['goods_attr_scoresum'];
		$order_data = array();
		...
		$order_data['memo'] =  strim($_REQUEST['memo']);

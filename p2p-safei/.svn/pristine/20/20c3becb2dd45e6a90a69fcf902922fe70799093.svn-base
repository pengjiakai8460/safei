...
class uc_lend
{
	public function index(){
		...
		if ($user_id >0){
			...
			$list = $GLOBALS['db']->getAll("SELECT dl.*,d.rate,d.repay_time,d.repay_time_type,d.deal_status,d.name FROM ".DB_PREFIX."deal_load dl LEFT JOIN ".DB_PREFIX."deal d ON dl.deal_id = d.id WHERE dl.user_id=".$user_id." order by dl.create_time desc ");
			...
			foreach($list as $k=>$v){
				...
				//$deal['url'] = $durl;
				$durl = "/index.php?ctl=deal&act=mobile&is_sj=1&id=".$v['deal_id'];

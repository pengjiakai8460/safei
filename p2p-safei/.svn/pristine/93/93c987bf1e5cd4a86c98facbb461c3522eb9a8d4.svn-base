<?php
	...
	function RegisterCretansfer($transfer_id,$t_user_id, $MerCode,$cert_md5,$post_url){
		...
		if ($GLOBALS['db']->affected_rows()){
			...
			$data['pMerBillNo'] = $transfer_id.'T'.TIME_UTC;//'商户订单号 否 商户系统唯一不重复',
			$data['pMerDate'] = to_date(TIME_UTC,'Ymd');//'商户日期 否 格式：YYYYMMDD ',
			...
			//债权面额 = 剩余本金 + 剩余利息  或 债权面额 = 转让金额，这样就不受 债权面额 限制了
			$data['pCretAmt'] = str_replace(',', '', number_format($transfer['transfer_amount'],2));//  '债权面额 否 金额单位元，不能为负，不允许为0 ',

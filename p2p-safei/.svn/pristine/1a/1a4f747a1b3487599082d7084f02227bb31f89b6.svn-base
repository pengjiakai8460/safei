...
class Ucenter_integrate implements integrate {
	...
	{		
		...
		if($uid>0)
		{
				...
				$ease_user = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where integrate_id = ".$uid." and user_name = '".$uname."'");
				...
				elseif(!$ease_user)
				{
					if(!$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where user_name = '".$uname."'")>0) 
					{
						...
						if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where email_encrypt = AES_ENCRYPT('".$email."','".AES_DECRYPT_KEY."')")>0)  //会员邮箱已存在时邮箱留空
						{
							$email = ''; 
						}
						//本系统没有该会员，在本站创建会员
		    ...
		    else
		    {
		        ...
		        else
		        {
					...
					foreach($ucconfig as $k=>$v)
					{
						...
						if($key_name[$k]=="UC_KEY"){
							$config_str.="define('".$key_name[$k]."','".htmlspecialchars($v)."');\n";
						}
						else{
	...
	private function import_user()
	{
		if(file_exists(APP_ROOT_PATH."public/uc_config.php"))
		{
		   ...
		   $user_list = get_user_info("*"," 1=1 ORDER BY `id` ASC","ALL");
		    foreach($user_list as $kk=>$data)
		    {
			    if(UC_CHARSET!='utf-8')
				{				

...
class deal_mobile
{
	public function index(){
		...
		//留言
		$message_list = $GLOBALS['db']->getAll("SELECT a.title,a.content,a.create_time,a.rel_id,a.user_id,a.is_effect,b.user_name FROM ".DB_PREFIX."message as a left join ".DB_PREFIX."user as b on  a.user_id = b.id WHERE a.rel_table= 'deal' AND a.rel_id = ".$id);
		$root['message']= $message_list;


		if ($deal['view_info'] != "") {
			$root['view_info_list'] = unserialize($deal['view_info']);
		}

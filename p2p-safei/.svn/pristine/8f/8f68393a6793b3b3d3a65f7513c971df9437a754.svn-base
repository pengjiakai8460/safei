...
class weixinModule extends SiteBaseModule
{
	...
	public function valid_url(){
		...
		$return_url=get_domain().url("index","weixin#platform_get_auth_code",array("type"=>0,"user_id"=>$GLOBALS['user_info']['id']));
	...
	public function gz_accept(){
		...
		if($result['status']==1){
			$msg=$result['info'];
			@file_put_contents(APP_ROOT_PATH."/public/wxt.txt",print_r($msg,1),FILE_APPEND);

...
class IpslogAction extends CommonAction{
	public function create()
	{
		...
		{
			$where = " where ((t.is_callback = 1 and t.code ='1') or (ept.is_callback = 1 and ept.code ='1')) and u.ips_acct_no > 0 ";
			//姓名
			if(strim($_REQUEST['user_name'])!='')
			{		
			...
			if(strim($_REQUEST['ident_no'])!='')
			{		
				$where .= " and u.idno like '%".strim($_REQUEST['ident_no'])."%'";
			}
			//手机号
			if(strim($_REQUEST['mobile'])!='')
			{		
				$where .= " and u.mobile like '%".strim($_REQUEST['mobile'])."%'";
			}
			if(strim($start_time)!="")
			{
				//$where .= " and t.create_time >=".to_timespan(strim($start_time));
				$where .= " and (t.create_time >=".to_timespan(strim($start_time))." or ept.create_time >=".to_timespan(strim($start_time)).")";
			}
			if(strim($end_time) !="")
			{
				$where .= " and (t.create_time <=".  to_timespan(strim($end_time))." or ept.crrete_time <= ".to_timespan(strim($start_time)).")";
			}
			
			$list = $GLOBALS["db"]->getAll("select u.id as user_id, u.user_name,u.user_type,u.mobile as pMobileNo,u.idno as pIdentNo ,u.real_name as pRealName,u.email as pEmail,t.create_time as t_create_time,ept.create as ept_create_time,t.idCardType as pIdentType ,t.id as t_id ,ept.id as ept_id
			from ".DB_PREFIX."user u left join ".DB_PREFIX."yeepay_register t on u.id = t.platformUserNo left join ".DB_PREFIX."yeepay_enterprise_register ept on u.id = ept.platformUserNo ".$where ." order by u.id desc ".$limit);
			...
			foreach($list as $k => $v)
			{
				if($v["user_type"] == 0)
				{
					...
					{
						$list[$k]["pIdentType"] = "二代身份证";
					}
					$v["pSmDate"] = $v['t_create_time'];
					
				}
				else
				{
					$list[$k]["id"] = $v["ept_id"];
					if($v['activeStatus'] == 'ACTIVATED')
						$list[$k]["pStatus"] = "已激活";
					else
						$list[$k]["pStatus"] = "未激活";
			...
			$list_count = $GLOBALS['db']->getOne("select count(*) 
			from ".DB_PREFIX."user u left join ".DB_PREFIX."yeepay_register t on u.id = t.platformUserNo left join ".DB_PREFIX."yeepay_enterprise_register ept on u.id = ept.platformUserNo ".$where);
		...
		$page = new Page($list_count,C("PAGE_LISTROWS"));   //初始化分页对象 
	...
	public function trade()
	{
		...
		elseif(strtolower($className) == "yeepay" || strtolower($className) == "baofoo")
		{
			...
			(services_fee / 100 * borrow_amount) as pLendFee,AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,
			repay_time as pTrdCycleValue,d.rate,
			repay_time_type as pTrdCycleType,d.loantype,
			...
			foreach($list as $k => $v)
			{
				...
				$list[$k]['pRepayMode'] = loantypename($v['loantype'],1);
	...
	public function trade_export_csv($page = 1)
	{
		...
		elseif(strtolower($className) == "yeepay" || strtolower($className) == "baofoo")
		{
			...
			$list = $GLOBALS['db']->getAll("select d.id,name, d.id as pBidNo,d.id as deal_id,deal_status,
			borrow_amount as pLendAmt,guarantees_money as pGuaranteesAmt,
			(services_fee / 100 * borrow_amount) as pLendFee,AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pRealName,
			repay_time as pTrdCycleValue,
			repay_time_type as pTrdCycleType,
			FROM_UNIXTIME( d.create_time, '%Y-%m-%d') as pRegDate,d.loantype
			 from ".DB_PREFIX."deal d left join ".DB_PREFIX."user u on d.user_id = u.id ".$where ." order by d.id desc ".$limit);
			$list_count = $GLOBALS['db']->getOne("select count(*)
			FROM_UNIXTIME( d.create_time, '%Y-%m-%d') as pRegDate
			 from ".DB_PREFIX."deal d left join ".DB_PREFIX."user u on d.user_id = u.id ".$where);
	...
	public function creditor()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			fee as pFee, paymentAmount as pTransferAmt, tenderAmount as pAuthAmt,AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$list = $GLOBALS['db']->getAll("select a.id, b.name as deal_name,
			a.cus_id as pBidNo,load_amount as pTrdAmt, u.user_name, AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,
	...
	public function creditor_export_csv($page = 1)
	{
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$list = $GLOBALS['db']->getAll("select a.id, b.name as deal_name,
			a.cus_id as pBidNo,(load_amount-fee) as pTrdAmt, u.user_name, AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,
			FROM_UNIXTIME(a.req_time/1000,'%Y-%m-%d') as pMerDate,
			fee as pFee, amount as pTransferAmt, b.borrow_amount as pAuthAmt,
			FROM_UNIXTIME(a.req_time/1000 ,'%Y-%m-%d')as pIpsTime,
			1 as pRegType
	...
	*/
	...
	public function recharge()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			$list = $GLOBALS['db']->getAll("select  requestNo as pMerBillNo,AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,u.user_name,yr.id,u.user_type,
			AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName, FROM_UNIXTIME( yr.create_time, '%Y-%m-%d' ) as pTrdDate,
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$list = $GLOBALS['db']->getAll("select order_id as pMerBillNo,AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,u.user_name,yr.id,u.user_type,
			AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName, FROM_UNIXTIME( yr.create_time, '%Y-%m-%d' ) as pTrdDate,
	...
	public function transfer()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			$y_sql = "select yw.id, requestNo  as pIpsBillNo,AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,u.user_name,u.user_type,1 as pOutType,
			FROM_UNIXTIME(yw.create_time ,'%Y-%m-%d')as pDwDate,amount as pTrdAmt,fee as pMerFee , 
			AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$y_sql = "select yw.id, order_id as pIpsBillNo,AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName,u.user_name,u.user_type,1 as pOutType,
			FROM_UNIXTIME(yw.create_time ,'%Y-%m-%d')as pDwDate,amount as pTrdAmt,fee as pMerFee , 
			AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,

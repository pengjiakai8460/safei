...
class uc_incharge
{
	...
	public function index(){
		...
		if($GLOBALS['db']->getOne("SELECT user_id FROM ".DB_PREFIX."user_credit_file WHERE user_id=".$user_id." AND passed=1")==false){
			$root['incharge_html']=2;
		}
		if ($user_id >0){
			...
			//输出支付方式
			if((int)$GLOBALS['request']['is_sdk']==1){
				$not_in = "'Wwxjspay'";
				...
				if((int)$GLOBALS['request']['is_weixin']==0){
			...
			foreach($payment_list as $k=>$v){
				...
				$payment_list[$k]['img'] = SITE_DOMAIN.APP_ROOT.$v['logo'];
				$payment_list[$k]['img'] = str_replace("/mapi.", "", $payment_list[$k]['img']);
			}
			
			$bank_card = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_bank where user_id = ".$user['id']." limit 1");
			if($user['is_bank'] == 1)
			{
				$root['bank_card'] = $bank_card['bankcard'];	//银行卡
			}
			else
			{
				$root['bank_card'] = '';	//银行卡
			...
			//var_dump($user['idcardpassed'],$user['idno'],$user['real_name']);exit;
			if($user['idcardpassed'] == 1)
			{
				$userinfo = $GLOBALS['db']->getRow("SELECT idno,real_name FROM ".DB_PREFIX."user where id = ".$user['id']);
				$root['idno'] = $userinfo['idno'];//身份证号
				$root['real_name'] = $userinfo['real_name'];
			}
			else
			{
				$root['idno'] = '';//身份证号
			...
			$this->wite_text(json_encode($user), dirname(__FILE__) . "/userinfo.txt");
			if (intval(app_conf("OPEN_IPS")) > 0){
				...
				if($GLOBALS['request']['from']=="wap"){
					$pTrdAmt = floatval($GLOBALS['request']['pTrdAmt']);
					$app_url = APP_ROOT."/index.php?ctl=collocation&act=DoDpTrade&user_type=0&pTrdBnkCode=parm_bnk&pTrdAmt=".$pTrdAmt."&user_id=".$user_id."&from=".$GLOBALS['request']['from'];
				}else{

...
function licai_item_format($vo){
	...
	if($vo['product_size']!=0){
		if($vo['product_size']>0&&$vo['product_size']<10000){
			$vo['product_size_format_num']=floatval($vo['product_size']).'元'; 
		}
		elseif($vo['product_size']>=10000&&$vo['product_size']<100000000){
	...
	$vo['url'] = url("index","licai#deal",array("id"=>$vo['id']));
	...
	if($vo['type'] >0 )
	{
		...
		//预热结束时间,有结束时间
		if($vo['time_limit'] == 0 && to_timespan($vo['begin_interest_date'])>to_timespan($vo['end_date']))
			$vo['before_interest_enddate'] = $vo['end_date'];
		else
		...
		elseif((to_timespan($vo['end_date']) == 0 || $vo['end_date'] == "0000-00-00") && $vo['time_limit'] > 0){
			//无项目结束时间 有周期
			$vo['end_interest_date'] = to_date(next_month(to_timespan($vo['begin_interest_date']),$vo['time_limit']),"Y-m-d");
		}
		...
		if(to_timespan($vo['end_buy_date'])>=to_timespan($vo["end_date"]))
...
function licai_bid($id,$money,$paypassword){
	...
	$licai['url'] = url("index","licai#deal",array("id"=>$licai['id']));
	...
	if(to_timespan($licai['end_buy_date'],"Y-m-d") > 0 && to_timespan($licai['end_buy_date'],"Y-m-d") <  to_timespan(to_date(TIME_UTC,"Y-m-d"))){
		$return['info'] = "理财产品已结束购买";
		return $return;
	}
	
	if(to_timespan($licai['end_date'],"Y-m-d") > 0 && to_timespan($licai['end_date'],"Y-m-d") <  to_timespan(to_date(TIME_UTC,"Y-m-d"))){
		$return['info'] = "理财产品已结束";
		return $return;
	}	
	...
	$licai_limit = $GLOBALS["db"]->getOne("select licai_limit from ".DB_PREFIX."licai where id = ".$id);
	
	$have_money = $GLOBALS["db"]->getOne("select sum(money) from ".DB_PREFIX."licai_order where status > 0 and user_id = ".$GLOBALS["user_info"]["id"]." and licai_id = ".$id." and create_date = '".to_date(TIME_UTC,"Y-m-d")."'");
	
	$total_money = $have_money + $money;
	
	if(floatval($licai_limit) > 0 && $total_money > floatval($licai_limit))
	{
		$return['info'] = "当前理财项目当日上限：".round($licai_limit,2)."，已理财金额为：".round($have_money,2).",当前投资金额为：".round($money,2)."，请调整后重新理财。";
		return $return;
	}
	...
	$licai['begin_interest_date'] = (to_timespan($licai['begin_interest_date'])==0 || to_timespan($licai['begin_interest_date']) < TIME_UTC) ? to_date(TIME_UTC,"Y-m-d") : $licai['begin_interest_date'] ;
	...
	if($GLOBALS['db']->affected_rows()){
		...
		{	
			//理财冻结资金
			modify_account(array("mortgage_money"=>$licai_order_data['freeze_bond']),intval($licai['user_id']),$GLOBALS['user_info']['user_name']."对理财“<a href=\"".$licai['url']."\">".$licai['name']."</a>”的投资,理财冻结资金",44);
		}
 		...
 		if($redempte['status']==0){
			if($status==1){
				...
				if(true){
					...
					$order=$GLOBALS['db']->getRow("select dorder.id,dorder.money,dorder.site_buy_fee,dorder.redempte_money,dorder.user_id as consumer_id,d.user_id as organiser_id,d.name as licai_name,d.id as licai_id,d.service_fee_rate,dorder.freeze_bond from ".DB_PREFIX."licai_order as dorder left join ".DB_PREFIX."licai as d on dorder.licai_id=d.id  where dorder.id=".$redempte['order_id']);
					...
					/*if($order['service_fee_rate']>0){
 					...
 					//$money=$redempte['money']+$organiser_fee+$earn_money;
					$licai['url'] = url("index","licai#deal",array("id"=>$order['licai_id']));
					...
					if($order['consumer_id']&&$order['organiser_id']){
						...
						//单笔保证金优先抵扣
					 	if($order['freeze_bond'] >= $money){
							//赎回本金
							modify_account(array('mortgage_money'=>'-'.$redempte['money']),$order['organiser_id'],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
							//赎回手续费
							modify_account(array('mortgage_money'=>'-'.$earn_money),$order['organiser_id'],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",40);
							//赎回平台手续费
							modify_account(array('mortgage_money'=>'-'.$organiser_fee),$order['organiser_id'],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",45);
							
							$GLOBALS['db']->query("update ".DB_PREFIX."licai_order set freeze_bond=freeze_bond-".$money." where id=".$order['id']);
							
					 	}
						elseif(($organiser_user['money'] + $order['freeze_bond'])>=$money){
							//单笔保证金和余额足够支付
							
					 		$ye_money = $money - $order['freeze_bond'];
							//保证金足够赎回本金
							if($order['freeze_bond'] > $redempte['money'])
							{
					 			modify_account(array('mortgage_money'=>'-'.$redempte['money']),$order["organiser_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
							}
							else
							{
								//保证金已经不够赎回本金，先扣保证金
								modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($redempte['money'] - $order['freeze_bond'])),$order["organiser_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
							}
							
							$order['freeze_bond'] = $order['freeze_bond'] - $redempte['money'];

							if($order['freeze_bond']<0)
							{
								$order['freeze_bond'] = 0;
							}
							//保证金足够支付利息
							if($order['freeze_bond'] > $earn_money)
							{
								//赎回收益
								modify_account(array('mortgage_money'=>'-'.$earn_money),$order["organiser_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",40);
							}
							...
							{
								//保证金不够支付利息，先扣保证金
								modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($earn_money - $order['freeze_bond'])),$order["organiser_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
							}
							
							$order['freeze_bond'] = $order['freeze_bond'] - $earn_money;
							...
							//平台手续费
							modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($organiser_fee - $order['freeze_bond'])),$order["organiser_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",45);
							
							$GLOBALS['db']->query("update ".DB_PREFIX."licai_order set freeze_bond = 0 where id=".$order['id']);
					 	}
						elseif($organiser_user['money']>=$money){
							//保证金没了,全部余额支付
					 		...
					 		if($pay_type==1){
						 		...
						 		if($re){
									...
									return $info;
						 			...
						 			$advance['real_money'] = $organiser_user['money'] + $order['freeze_bond'];
						 			...
						 			if(!$re){
										...
										//垫付的钱充入账户
										modify_account(array('money'=>$advance['advance_money']),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”,网站垫付资金",48);					
										//冻结资金够支付赎回本金
										if($order['freeze_bond']>$redempte['money'])
										{
											modify_account(array('mortgage_money'=>'-'.$redempte['money']),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
										}
										//冻结资金不够支付赎回本金
										else
										{
											
											modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($redempte['money']-$order['freeze_bond'])),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",39);
										}
										...
										{
											//赎回收益
											modify_account(array('mortgage_money'=>'-'.$earn_money),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",40);
										}
										else
										{
											//赎回收益
											modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($earn_money-$order['freeze_bond'])),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",40);
										}
										...
										//赎回平台手续费
										modify_account(array('mortgage_money'=>'-'.$order['freeze_bond'],'money'=>'-'.($organiser_fee-$order['freeze_bond'])),$advance["user_id"],"发放用户要赎回的理财产品“<a href=\"".$licai['url']."\">".$licai['name']."</a>”",45);
...
function get_licai_status($id)
{
	...
	$time = get_peizi_next_date(to_timespan($vo["begin_interest_date"]),$vo['begin_interest_type']);

	if(TIME_UTC < to_timespan($time))
	{
		$status = 0;
	}
	elseif(($vo["end_date"] =="" || $vo["end_date"] = "0000-00-00" || TIME_UTC <= to_timespan($vo["end_date"])) && TIME_UTC >= to_timespan($time) && $time >0)
	{
		$status = 1;
	}

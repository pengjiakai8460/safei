	...
	public function dologin()
	{
		...
		{	
			...
			{	
				...
				$jump_url = url("debit","debit_uc_center");
	...
	*/
	public function loginout()
	{
		...
		if($result['status'])
		{
			...
			$before_loginout = $redirect = url("debit","debit");
	...
	public function send_password()
	{
		...
		if($GLOBALS['db']->getAll("select count(*) from ".DB_PREFIX."user where email_encrypt = AES_ENCRYPT('".$email."','".AES_DECRYPT_KEY."')") == 0)
		{
		...
		if($user_info = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where email_encrypt = AES_ENCRYPT('".$email."','".AES_DECRYPT_KEY."')"))
		{
			$result = 1;  //初始为1
			//载入会员整合
	...
	{	
		...
		if($user_info = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where mobile_encrypt =AES_ENCRYPT('".$mobile."','".AES_DECRYPT_KEY."')"))
		{
			$result = 1;  //初始为1
			//载入会员整合
			/*$integrate_code = trim(app_conf("INTEGRATE_CODE"));
	...
	public function do_re_name_id()
	{
		...
		//判断该实名是否存在
		if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."user where idno_encrypt = AES_DECRYPT('.$idno.','".AES_DECRYPT_KEY."') and id<> $id ") > 0 ){
			showErr("该实名已被其他用户认证，非本人请联系客服");
		}
		
		
		if($user = $GLOBALS['db']->getRow("select *,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(email_encrypt,'".AES_DECRYPT_KEY."') as email,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile from ".DB_PREFIX."user where id =".$id))
		{	
			$user_info_re = array();
			$user_info_re['id'] = $id;

...
class Yjwap_payment implements payment {
	...
	public function response($request)
	{
		...
		//file_put_contents("./log/yjwap_response_".strftime("%Y%m%d%H%M%S",time()).".txt",print_r($request,true));
    	...
    	try {
    		...
    		*/
    		...
    		if ($return['status'] == 1){   		
	    		...
	    		{
	    			showIpsInfo("支付完成",wap_url("member","uc_incharge_log#index"));
	    		}
    		}else{
    		...
    		// TODO：添加订单支付异常逻辑代码
    		showIpsInfo("验证失败",wap_url("member","uc_incharge#index"));
    	} 
	}

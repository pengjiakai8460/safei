...
class VipSettingAction extends CommonAction {
   ...
   public function index() {
	...
	public function recharge_config(){
		$vip_id = intval($_REQUEST ['vip_id']);
		if($vip_id>0) {
			$list = M()->query("SELECT * FROM ".DB_PREFIX."user_recharge_config WHERE vip_id='$vip_id' ORDER BY id ASC");
		}else{
			$list = M()->query("SELECT * FROM ".DB_PREFIX."user_recharge_config ORDER BY id ASC");
		}
		
		$this->assign("vip_list",M("VipType")->where('is_effect = 1 and is_delete = 0 order by sort ')->findAll());
		$this->assign("list",$list);
		
		$this->assign("payment_list",M("Payment")->where('is_effect = 1 and name<>"" order by sort ')->findAll());
	...
	public function saverechargeconfig(){
		...
		M()->query("DELETE FROM ".DB_PREFIX."user_recharge_config WHERE id not in (".implode(",",$has_ids).")");
		...
		foreach($config['id'] as $k=>$v){
			if(intval($v) > 0){
				...
				$config_data['id'] = $v;
				$config_data['name'] = trim($config['name'][$k]);
				$config_data['interface_class'] = trim($config['interface_class'][$k]);
				$config_data['fee'] = floatval($config['fee'][$k]);
				$config_data['vip_id'] = intval($config['vip_id'][$k]);
				$config_data['fee_type'] = intval($config['fee_type'][$k]);
				M("UserRechargeConfig")->save($config_data);
			}
		}
		...
		foreach($aconfig['name'] as $k=>$v){
			if(trim($v)!=""){
				...
				$config_data['vip_id'] = intval($aconfig['vip_id'][$k]);
				$config_data['fee_type'] = intval($aconfig['fee_type'][$k]);
				M("UserRechargeConfig")->add($config_data);
			}
		}
		rm_auto_cache("user_recharge_config");
		parent::success(L("UPDATE_SUCCESS"));
	}

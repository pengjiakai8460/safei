...
class CreateRelevanceAction extends CommonAction{
	...
	{	
		...
		$pid_name  = strim($_REQUEST['pid_name']);
		...
		$condition = " if(u.pid >0, ru.user_type in (0,1) ,'1=1') and u.user_type IN (0,1) ";
		if($user_name != "" )
			$condition.=" and u.user_name like '%".$user_name."%' ";
		if($pid_name != "" )
			$condition.=" and u.pid in(select id from ".DB_PREFIX."user where user_name='".$pid_name."') ";
		if(isset($_REQUEST['pid']) && intval($_REQUEST['pid']) > 0){
			$condition.=" and u.pid = ".intval($_REQUEST['pid']);
		}
		...
		if($count>0){
			$sql = "SELECT u.id,u.user_name,AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') as email,u.pid,u.referer_memo,u.referral_rate FROM ".DB_PREFIX."user u left join ".DB_PREFIX."user ru ON ru.id =u.pid WHERE  $condition  ORDER BY $order $sort LIMIT ".($p->firstRow . ',' . $p->listRows);
	...
	public function export_csv($page = 1)
	{
		...
		$sql = "SELECT u.id,u.user_name,AES_DECRYPT(u.email_encrypt,'".AES_DECRYPT_KEY."') as email,u.pid,u.referer_memo,u.referral_rate FROM ".DB_PREFIX."user u left join ".DB_PREFIX."user ru ON ru.id =u.pid WHERE  $condition LIMIT ".$limit;
		$list = $GLOBALS['db']->getAll($sql);
		foreach($list as $k=>$v){
			if($list[$k]['pid'] == 0){
				$list[$k]['status']="未关联";

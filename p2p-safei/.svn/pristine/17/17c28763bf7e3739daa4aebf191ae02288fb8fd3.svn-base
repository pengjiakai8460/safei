	...
	$("select[name='uloadtype']").change(function(){
		...
		switch(val){
			case "0":
				$(".uloadtype_0").show();
				$(".uloadtype_1").hide();
				$(".uloadtype_1 input[name='portion']").removeClass("require");
				break;
			case "1":
				$(".uloadtype_0").hide();
				$(".uloadtype_1").show();
				$(".uloadtype_1 input[name='portion']").addClass("require");
				if($(".uloadtype_1 input[name='portion']").val()=="0"){
					$(".uloadtype_1 input[name='portion']").val("");
				}
				break;
		}
	});
...
function add_mortgage_img(obj){
	...
	Obj_img_add[idx] = $("#mortgage_"+obj+"_box .keimg").length + 1;
	if(Obj_img_add[idx] > 20){
		alert('做多只能添加20个');
		return ;
	}
	
	var html = $("#J_tmp_ke_box").html();
		html = html.replace(/%k/g,obj);
		html = html.replace(/%s/g,Obj_img_add[idx]);
        $("#mortgage_"+obj+"_box").append(html);
		bindKdupload();
}


function load_img_items(imgs,obj){
	var img_add = 1;
	$.each(imgs,function(i,v){
		var sobj = $("#J_tmp_ke_box").html();
		sobj = sobj.replace(/%k/g,obj);
		sobj = sobj.replace(/%s/g,img_add);
		$("#mortgage_"+obj+"_box").append(sobj);
		
		$("#mortgage_"+obj+"_box .keimg_d").show();
		$("#mortgage_"+obj+"_name_"+img_add).val(v['name']);
		$("#keimg_h_mortgage_"+obj+"_img_"+img_add+"_i").val(v['img']);
		$("#keimg_a_mortgage_"+obj+"_img_"+img_add).attr("href",v['img'])
		$("#keimg_m_mortgage_"+obj+"_img_"+img_add).attr("src",v['img'])

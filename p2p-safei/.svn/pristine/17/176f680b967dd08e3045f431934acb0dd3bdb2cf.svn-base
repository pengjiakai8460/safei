...
class Sqepay_payment implements payment {
    ...
    public function get_payment_code($payment_notice_id) {
        ...
        $ReturnURL = SITE_DOMAIN.APP_ROOT.'/callback/pay/95epay_response.php';
        //后台通知
        $NotifyURL = SITE_DOMAIN.APP_ROOT.'/callback/pay/95epay_notify.php';
    ...
    public function response($request) {
       ...
       if($MD5info == $md5sign && $Succeed=="88"){
        	...
        	showErr($GLOBALS['payment_lang']["VALID_ERROR"]." ".$request['Result']);
        }
    }
     ...
     public function get_display_code() {
		...
		if($payment_item)
		{
	        ...
	        $html = "<style type='text/css'>.sqepay_types{float:left; display:block; background:url(".SITE_DOMAIN.APP_ROOT."/system/payment/Sqepay/banklist_hnapay.jpg); font-size:0px; width:150px; height:10px; text-align:left; padding:15px 0px;}";
			...
			$html .="}</script>";
			$html.= "<h3 class='clearfix tl'><b>双乾直连</b></h3><div class='blank1'></div><hr />";
	       foreach ($payment_cfg['sqepay_gateway'] AS $key=>$val)
	        {
	        	if($key=="0")
	        	{
	        		$html  .= "<label class='sqepay_types sqbk_type_".$key." ui-radiobox' rel='common_payment'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick='set_bank(\"\")' /></label>";
	        	}
	        	else
	           	 $html  .= "<label class='sqepay_types sqbk_type_".$key." ui-radiobox' rel='common_payment'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick='set_bank(\"".$key."\")' /></label>";
	        }

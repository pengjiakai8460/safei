...
class uc_ipsModule extends SiteBaseModule
{
	...
	public function recharge()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			$list = $GLOBALS['db']->getAll("select requestNo as pMerBillNo,AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$list = $GLOBALS['db']->getAll("select order_id as pMerBillNo,AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,
			AES_DECRYPT(u.real_name_encrypt,'".AES_DECRYPT_KEY."') as pRealName, FROM_UNIXTIME( yr.create_time, '%Y-%m-%d' ) as pTrdDate,
	...
	public function transfer()
	{
		...
		if(strtolower($className) == "ips")
		{
			...
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."ips_do_dw_trade ".$where." ORDER BY pDwDate DESC ".$limit);
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			$y_sql = "select requestNo  as pIpsBillNo,AES_DECRYPT(u.real_name,'".AES_DECRYPT_KEY."') as pRealName,
			...
			case when feeMode ='PLATFORM' then 1 else 2 end as pIpsFeeType
			 from ".DB_PREFIX."yeepay_withdraw yw left join ".DB_PREFIX."user u on yw.platformUserNo = u.id ".$where." ORDER BY pDwDate DESC ".$limit;
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			$y_sql = "select order_id as pIpsBillNo,AES_DECRYPT(u.real_name,'".AES_DECRYPT_KEY."') as pRealName,
			FROM_UNIXTIME(yw.create_time ,'%Y-%m-%d')as pDwDate,amount as pTrdAmt,fee as pMerFee , 
			AES_DECRYPT(u.idno_encrypt,'".AES_DECRYPT_KEY."') as pIdentNo,
			fee_taken_on as pIpsFeeType
			 from ".DB_PREFIX."baofoo_fo_charge yw left join ".DB_PREFIX."user u on yw.user_id = u.id ".$where." ORDER BY pDwDate DESC ".$limit;
			$list = $GLOBALS['db']->getAll($y_sql);
	...
	public function repayment()
	{
		...
		elseif(strtolower($className) == "yeepay")
		{
			...
			if(strim($start_time)!="")
			{
				$condition .= " and yct.create_time >=".to_timespan(strim($start_time));
			}
			if(strim($end_time) !="")
			{
				$condition .= " and yct.create_time <=".  to_timespan(strim($end_time));
			}
			$list = $GLOBALS['db']->getAll($sql.$condition." order by mid desc ".$limit);
			$list_count = $GLOBALS['db']->getOne($count_sql.$condition);
		}
		...
		elseif(strtolower($className) == "baofoo")
		{
			...
			if(strim($start_time)!="")
			{
				$condition .= " and yct.req_time >=".to_timespan(strim($start_time));
			}
			if(strim($end_time) !="")
			{
				$condition .= " and yct.req_time <=".  to_timespan(strim($end_time));
			}
	...
	public function receivedpayment()
	{
		...
		elseif(strtolower($className) == "baofoo")
		{
			$condition = " where deal_t.code='CSD000' AND (r.t_user_id = ".$user_id ." or (r.user_id = ".$user_id ." and r.t_user_id = 0)) ";

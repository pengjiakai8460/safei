...
class PromoteMsgAction extends CommonAction{
	...
	//开始验证
		...
		switch($msg_data['send_type'])
		{
			...
			case 3:
				//信用等级
				$msg_data['send_type_id'] = intval($_REQUEST['level_id']);
				break;
			case 4:
				//VIP等级
				$msg_data['send_type_id'] = intval($_REQUEST['vip_id']);
				break;
...
//					$condition .= " and group_id = ".$msg_data['send_type_id'];
				...
				case 1:
					...
					$mail_list = M("MailList")->where($condition)->field("mail_address")->limit($limit)->findAll();
					$email = '';
					foreach($mail_list as $k=>$v)
					{
				...
				case 3:
					...
					$condition = "is_effect = 1 and is_delete = 0";
					if($msg_data['send_type_id']>0)					
					$condition .= " and level_id = ".$msg_data['send_type_id'];
					$mail_list = M("User")->where($condition)->field("email")->limit($limit)->findAll();					
					$email = '';
					foreach($mail_list as $k=>$v)
					{
				...
				case 4:
					...
					$condition = "is_effect = 1 and is_delete = 0";
					if($msg_data['send_type_id']>0)					
					$condition .= " and vip_id = ".$msg_data['send_type_id'];
					$mail_list = M("User")->where($condition)->field("email")->limit($limit)->findAll();					
					$email = '';
					foreach($mail_list as $k=>$v)
					{

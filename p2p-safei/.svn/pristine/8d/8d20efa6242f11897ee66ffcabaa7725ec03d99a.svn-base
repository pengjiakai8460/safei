...
class fuiouwap_payment implements payment {
    ...
    public function get_payment($payment_notice_id) {
        ...
        $data['mchntcd'] = trim($payment_info['config']['fuiou_account']);  // 商户代码
        //$data['mchntcd']    = '0001000F0358674';  // 商户代码
        ...
        $data['xml_logotp'] = '0';
        $data['xml_mchnt_order_id'] = $payment_notice['notice_sn'];     // 订单号
        $data['xml_user_id'] = $payment_notice['user_id'];       // 用户ID
        $data['xml_amt'] = $money * 100;                     // 交易金额
        $data['xml_bank_card'] = $bankcard;               // 银行卡号
        ...
        $data['xml_name'] = $uname;                    // 用户真实姓名
        $data['xml_id_type'] = '0';                              // 0：证件类型默认身份证
        $data['xml_id_no'] = $uidno;                    // 身份证号
        $data['xml_signtp'] = 'md5';                            // 加密方式
        $data['xml_sign'] = $this->WebMd5Sign($data);

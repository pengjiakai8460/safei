...
class IdcardAction extends CommonAction{
	private function read_modules()
	{
		$directory = APP_ROOT_PATH."system/idcard/";
		$read_modules = true;
	...
	public function index()
	{
		...
		$db_modules = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."idcard");
		...
		$this->assign("service_list",$modules);
		$this->display();
	}
	...
	public function install()
	{
		...
		if(file_exists($file))
		{
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."idcard set is_effect = 0 where class_name <> '".$class_name."'");
			$GLOBALS['db']->query("update ".DB_PREFIX."idcard set is_effect = 1 where class_name = '".$class_name."'");
			
			$rs = M("Idcard")->where("class_name = '".$class_name."'")->count();
			if($rs > 0)
			{
	...
	public function insert()
	{
		...
		if (false !== $list) {
			$GLOBALS['db']->query("update ".DB_PREFIX."idcard set is_effect = 0 where id <> '".$list."'");
			//成功提示
	...
	public function edit() {		
		...
		$file = $directory.$vo['class_name'].".class.php";
		if(file_exists($file))
		{
			$module = require_once($file);
		}
	...
	public function uninstall()
	{
		...
		if($data)
		{
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."idcard set is_effect = 0 where id = '".$id."'");

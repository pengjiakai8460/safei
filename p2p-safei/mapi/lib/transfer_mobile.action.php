...
class transfer_mobile
{
	public function index(){
		...
		$root['transfer_id'] = intval($GLOBALS['request']['transfer_id']);
		...
		//留言
		$message_list = $GLOBALS['db']->getAll("SELECT a.title,a.content,a.create_time,rel_id,a.user_id,a.is_effect,b.user_name FROM ".DB_PREFIX."message as a left join ".DB_PREFIX."user as b on  a.user_id = b.id WHERE a.rel_table='transfer' and a.rel_id = ".$root['transfer_id']);
		...
		*/
		...
		$condition = ' AND dlt.id='.$root['transfer_id'].' AND d.deal_status >= 4 and d.is_effect=1 and d.is_delete=0  and d.repay_time_type =1 and  d.publish_wait=0 ';
